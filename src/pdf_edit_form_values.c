...
static int quantapdf_pdf_edit_form_utf8_valid(
    const char *text,
    size_t size)
{
...
static int quantapdf_pdf_edit_form_live_contains(
    fz_context *ctx,
...
static quantapdf_status quantapdf_pdf_edit_form_text_preflight(
...
static quantapdf_status quantapdf_pdf_edit_form_validate_text_update(
    ...
    if (update->presence == QUANTAPDF_FORM_VALUE_MISSING) {
        ...
        *out_missing = 1;
        ...
        if (value->struct_size < value_min ||
            ...
            return QUANTAPDF_ERROR_ARGUMENT;
        if (!quantapdf_pdf_edit_form_utf8_valid(value->utf8, value->utf8_size))
            return QUANTAPDF_ERROR_ARGUMENT;
...
static int quantapdf_pdf_edit_form_text_is_noop(
...
static void quantapdf_pdf_edit_form_write_text(
    quantapdf_pdf_edit *edit,
    const quantapdf_pdf_form_live_field *live,
    int missing,
    const char *text)
    ...
    const quantapdf_form_value_update *update)
{
    ...
    status = quantapdf_pdf_edit_form_text_preflight(edit);
    if (status != QUANTAPDF_OK)
        return status;
    status = quantapdf_pdf_edit_form_validate_text_update(
        field, update, &text, &text_size, &missing);
    if (status != QUANTAPDF_OK)
        return status;
    if (missing && live->effective_v_present &&
        !quantapdf_pdf_edit_form_live_contains(
            edit->ctx, live, live->effective_v_owner)) {
        free(text);
        return QUANTAPDF_ERROR_UNSUPPORTED;
    }
    if (quantapdf_pdf_edit_form_text_is_noop(
    ...
    fz_try(edit->ctx)
    {
        ...
        operation_open = 1;
        quantapdf_pdf_edit_form_write_text(edit, live, missing, text);
#if defined(QUANTAPDF_TESTING)
        if (edit->test_fault ==
