...
typedef struct extractpdf_pdf_form_value_internal {
    extractpdf_form_value_kind kind;
    size_t option_index;
    extractpdf_pdf_form_string utf8;
} extractpdf_pdf_form_value_internal;

typedef struct extractpdf_pdf_form_option_internal {
    extractpdf_form_option_kind kind;
    extractpdf_pdf_form_string export_text;
    extractpdf_pdf_form_string display_text;
    char *button_state;
} extractpdf_pdf_form_option_internal;

typedef struct extractpdf_pdf_form_field_internal {
...
} extractpdf_pdf_form_field_internal;

typedef struct extractpdf_pdf_form_widget_internal {
    size_t field_index;
    int page_index;
    extractpdf_rect bounds;
    uint32_t flags;
    size_t button_option_index;
} extractpdf_pdf_form_widget_internal;

typedef struct extractpdf_pdf_form_model {
    extractpdf_pdf_form_field_internal *fields;
    size_t field_count;
    extractpdf_pdf_form_value_internal *values;
    size_t value_count;
    extractpdf_pdf_form_option_internal *options;
    size_t option_count;
    extractpdf_pdf_form_widget_internal *widgets;
    size_t widget_count;
    char *strings;
    size_t string_size;
