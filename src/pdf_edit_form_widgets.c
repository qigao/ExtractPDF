    ...
    quantapdf_pdf_edit_form_widget_handles *out_handles)
{
    ...
    out_handles->items = (quantapdf_pdf_edit_form_widget_handle *)calloc(
    ...
    fz_try(edit->ctx)
    {
        for (target = 0; target < live->widget_count && status == QUANTAPDF_OK; ++target) {
...
{
    int caught_code = FZ_ERROR_NONE;
    quantapdf_status restore_status;
    size_t i;
    ...
    quantapdf_pdf_edit_form_widget_handles *handles)
{
    ...
    for (i = 0; i < handles->count; ++i) {
        ...
        if (edit->test_fault ==
            QUANTAPDF_PDF_EDIT_TEST_FAULT_FORM_AFTER_FIRST_AP_REFRESH) {
