...
typedef struct quantapdf_pdf_edit_choice_assignment {
    int missing;
    int custom_utf8;
    char *utf8;
    size_t utf8_size;
    ...
    size_t value_count;
} quantapdf_pdf_edit_choice_assignment;

static void quantapdf_pdf_edit_choice_assignment_drop(
    quantapdf_pdf_edit_choice_assignment *assignment)
{
    if (assignment == NULL)
        return;
...
static int quantapdf_pdf_edit_choice_utf8_valid(const char *text, size_t size)
{
...
static int quantapdf_pdf_edit_choice_live_contains(
    fz_context *ctx,
...
static quantapdf_status quantapdf_pdf_edit_choice_validate_option(
    const quantapdf_pdf_form_field_internal *field,
    const quantapdf_form_value_input *value,
    size_t *out_index)
{
    size_t value_min = QUANTAPDF_FORM_VALUE_INPUT_V1_MIN_SIZE;
...
static quantapdf_status quantapdf_pdf_edit_choice_validate_utf8(
    const quantapdf_form_value_input *value,
    char **out_text,
    size_t *out_size)
{
    ...
    if (value->struct_size < value_min ||
        ...
        !quantapdf_pdf_edit_choice_utf8_valid(value->utf8, value->utf8_size))
        return QUANTAPDF_ERROR_ARGUMENT;
...
static quantapdf_status quantapdf_pdf_edit_choice_validate_update(
    const quantapdf_pdf_form_field_internal *field,
    const quantapdf_form_value_update *update,
    quantapdf_pdf_edit_choice_assignment *assignment)
{
    size_t update_min = offsetof(quantapdf_form_value_update, value_count) +
    ...
    } else if (field->type == QUANTAPDF_FORM_FIELD_LIST_BOX) {
...
static quantapdf_status quantapdf_pdf_edit_choice_validate_exports(
    const quantapdf_pdf_form_model *model,
    const quantapdf_pdf_form_field_internal *field,
    const quantapdf_pdf_edit_choice_assignment *assignment)
{
    size_t i;
    ...
    for (i = 0; i < assignment->value_count; ++i) {
        const quantapdf_pdf_form_option_internal *option = &model->options[
...
static int quantapdf_pdf_edit_choice_is_noop(
    const quantapdf_pdf_form_model *model,
    const quantapdf_pdf_form_field_internal *field,
    const quantapdf_pdf_edit_choice_assignment *assignment)
{
    ...
    for (i = 0; i < assignment->value_count; ++i) {
        ...
        if (value->kind != QUANTAPDF_FORM_VALUE_OPTION ||
...
static void quantapdf_pdf_edit_choice_delete_descendant_values(
    fz_context *ctx,
    const quantapdf_pdf_form_live_field *live)
{
    size_t i;
...
static const char *quantapdf_pdf_edit_choice_export(
    const quantapdf_pdf_form_model *model,
    const quantapdf_pdf_form_field_internal *field,
    size_t option_index)
{
...
static void quantapdf_pdf_edit_choice_write(
    ...
    const quantapdf_pdf_edit_choice_assignment *assignment)
{
    ...
    if (assignment->custom_utf8) {
        ...
        quantapdf_pdf_edit_choice_delete_descendant_values(edit->ctx, live);
        return;
    }

    if (field->type == QUANTAPDF_FORM_FIELD_LIST_BOX &&
        ...
        for (i = 0; i < assignment->value_count; ++i) {
            const char *text = quantapdf_pdf_edit_choice_export(
    ...
    const quantapdf_form_value_update *update)
{
    ...
    status = quantapdf_pdf_edit_choice_validate_update(field, update, &assignment);
    if (status != QUANTAPDF_OK)
        return status;
    status = quantapdf_pdf_edit_choice_validate_exports(model, field, &assignment);
    if (status != QUANTAPDF_OK) {
        quantapdf_pdf_edit_choice_assignment_drop(&assignment);
        return status;
    }
    if (assignment.missing && live->effective_v_present &&
        !quantapdf_pdf_edit_choice_live_contains(
            edit->ctx, live, live->effective_v_owner)) {
        quantapdf_pdf_edit_choice_assignment_drop(&assignment);
        return QUANTAPDF_ERROR_UNSUPPORTED;
    }
    if (quantapdf_pdf_edit_choice_is_noop(model, field, &assignment)) {
        quantapdf_pdf_edit_choice_assignment_drop(&assignment);
        return QUANTAPDF_OK;
    }

    status = quantapdf_pdf_edit_form_prepare_widget_handles(edit, live, &handles);
    if (status != QUANTAPDF_OK) {
        quantapdf_pdf_edit_choice_assignment_drop(&assignment);
        return status;
    }
    ...
    fz_try(edit->ctx)
    {
        ...
        operation_open = 1;
        quantapdf_pdf_edit_choice_write(
...
#endif
        quantapdf_pdf_edit_form_refresh_widget_handles(edit, &handles);
        if (quantapdf_pdf_edit_form_restore_widget_editing(edit, &handles) !=
            QUANTAPDF_OK)
            fz_throw(edit->ctx, FZ_ERROR_GENERIC,
    ...
    fz_catch(edit->ctx)
    {
        ...
        (void)quantapdf_pdf_edit_form_restore_widget_editing(edit, &handles);
