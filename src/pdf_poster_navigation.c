...
typedef struct poster_navigation_walk {
    ...
    const quantapdf_pdf_poster_private_split *runtime;
    int apply;
} poster_navigation_walk;

static quantapdf_pdf_poster_split_plan *find_split(
    quantapdf_pdf_poster_plan *plan,
    int page_index,
    size_t *out_index)
{
...
static quantapdf_status reserve_destinations(
    quantapdf_pdf_poster_plan *plan,
    size_t needed)
{
    quantapdf_pdf_poster_dest_plan *grown;
    size_t capacity;
    ...
    grown = (quantapdf_pdf_poster_dest_plan *)realloc(
...
static quantapdf_status choose_destination_tile(
...
static const quantapdf_pdf_poster_dest_plan *find_destination_plan(
    ...
    size_t owner_ordinal)
{
    ...
    for (i = 0; i < plan->destination_count; ++i) {
        const quantapdf_pdf_poster_dest_plan *entry = &plan->destinations[i];
...
static quantapdf_status process_destination(
    fz_context *ctx,
    ...
    size_t owner_ordinal)
{
    ...
    quantapdf_pdf_poster_split_plan *split;
    const quantapdf_pdf_poster_dest_plan *existing;
...

    {
        ...
        if (status != QUANTAPDF_OK)
            return status;
        {
            quantapdf_pdf_poster_dest_plan *entry =
...
static quantapdf_status process_links(
    fz_context *ctx,
    pdf_document *document,
    poster_navigation_walk *walk)
{
        ...
        for (index = 0; index < count; ++index) {
            ...
            if (dest != NULL) {
                status = process_destination(
                    ...
                    walk,
                    dest,
                    QUANTAPDF_PDF_POSTER_DEST_LINK_DIRECT,
                    page_index,
                    (size_t)index);
                if (status != QUANTAPDF_OK)
                    return status;
            ...
            if (action != NULL && pdf_is_dict(ctx, action) &&
                ...
                status = process_destination(
                    ...
                    walk,
                    target,
                    QUANTAPDF_PDF_POSTER_DEST_LINK_ACTION,
                    page_index,
                    (size_t)index);
                if (status != QUANTAPDF_OK)
                    return status;
...
static quantapdf_status outline_visit(
    fz_context *ctx,
    pdf_document *document,
    pdf_obj *item,
    ...
    void *user)
{
    ...
    if (dest != NULL)
        return process_destination(
            ...
            outline->walk,
            dest,
            QUANTAPDF_PDF_POSTER_DEST_OUTLINE_DIRECT,
            -1,
            preorder_index);
    if (action == NULL)
        return QUANTAPDF_OK;
    ...
    if (pdf_name_eq(ctx, kind, PDF_NAME(GoTo))) {
        ...
        return process_destination(
            ...
            outline->walk,
            target,
            QUANTAPDF_PDF_POSTER_DEST_OUTLINE_ACTION,
            -1,
            preorder_index);
    }
    return QUANTAPDF_ERROR_UNSUPPORTED;
...
static quantapdf_status process_outlines(
    fz_context *ctx,
    pdf_document *document,
    poster_navigation_walk *walk)
{
    ...
    size_t count = 0;
    outline.walk = walk;
    return quantapdf_pdf_outline_walk_strict(
        ctx, document, outline_visit, &outline, &count);
}

static quantapdf_status destination_from_definition(
    fz_context *ctx,
    pdf_obj *value,
...
static quantapdf_status process_name_tree_node(
    fz_context *ctx,
    pdf_document *document,
    poster_navigation_walk *walk,
    pdf_obj *node,
...
static quantapdf_status process_named_destinations(
    fz_context *ctx,
    pdf_document *document,
    poster_navigation_walk *walk,
    pdf_obj *root)
...
static quantapdf_status process_legacy_destinations(
    fz_context *ctx,
    pdf_document *document,
    poster_navigation_walk *walk,
    pdf_obj *root)
...
static quantapdf_status process_navigation_registry(
    fz_context *ctx,
    pdf_document *document,
    poster_navigation_walk *walk)
{
    ...
    quantapdf_pdf_poster_private_split *runtime)
{
    ...
    if (ctx == NULL || document == NULL || plan == NULL || runtime == NULL)
        return QUANTAPDF_ERROR_ARGUMENT;
    walk.plan = (quantapdf_pdf_poster_plan *)plan;
