...
static int poster_has_key(fz_context *ctx, pdf_obj *dict, pdf_obj *key)
{
    return pdf_dict_get(ctx, dict, key) != NULL;
}

static int poster_public_inside(extractpdf_rect inner, extractpdf_rect outer)
{
    return inner.x0 >= outer.x0 && inner.y0 >= outer.y0 &&
        inner.x1 <= outer.x1 && inner.y1 <= outer.y1;
}

static extractpdf_status poster_read_public_rect(
    fz_context *ctx,
    pdf_obj *annotation,
    ...
    extractpdf_rect *out_rect)
{
    ...
    if (!pdf_is_number(ctx, pdf_array_get(ctx, rect, 0)) ||
        !pdf_is_number(ctx, pdf_array_get(ctx, rect, 1)) ||
        !pdf_is_number(ctx, pdf_array_get(ctx, rect, 2)) ||
        !pdf_is_number(ctx, pdf_array_get(ctx, rect, 3)))
        return EXTRACTPDF_ERROR_FORMAT;

    x0 = pdf_to_real(ctx, pdf_array_get(ctx, rect, 0));
    y0 = pdf_to_real(ctx, pdf_array_get(ctx, rect, 1));
    x1 = pdf_to_real(ctx, pdf_array_get(ctx, rect, 2));
    y1 = pdf_to_real(ctx, pdf_array_get(ctx, rect, 3));
    ...
    int *out_crosses)
{
    ...
    for (index = 0; index < split->tile_count; ++index) {
        ...
        float x0 = fmaxf(rect.x0, tile.x0);
        float y0 = fmaxf(rect.y0, tile.y0);
        float x1 = fminf(rect.x1, tile.x1);
        float y1 = fminf(rect.y1, tile.y1);
        if (x0 < x1 && y0 < y1)
            ++count;
    }
    if (count == 0)
...
static extractpdf_pdf_poster_split_plan *poster_find_split(
...
static extractpdf_status poster_check_link_action(
    fz_context *ctx,
    pdf_obj *annotation)
{
    ...
    if (poster_has_key(ctx, annotation, PDF_NAME(AA)))
        return EXTRACTPDF_ERROR_UNSUPPORTED;
    ...
    if (!pdf_is_dict(ctx, action))
        return EXTRACTPDF_ERROR_FORMAT;
    if (poster_has_key(ctx, action, PDF_NAME(Next)))
        return EXTRACTPDF_ERROR_UNSUPPORTED;
    {
...
static int poster_page_key_allowed(fz_context *ctx, pdf_obj *key)
{
    return pdf_name_eq(ctx, key, PDF_NAME(Type)) ||
        ...
        pdf_name_eq(ctx, key, PDF_NAME(Tabs)) ||
        pdf_name_eq(ctx, key, PDF_NAME(Annots));
}

static extractpdf_status poster_check_selected_page_keys(
    fz_context *ctx,
    extractpdf_pdf_poster_split_plan *split)
{
    ...
    for (index = 0; index < count; ++index) {
        if (!poster_page_key_allowed(ctx, pdf_dict_get_key(ctx, page, index)))
            return EXTRACTPDF_ERROR_UNSUPPORTED;
    }
    return EXTRACTPDF_OK;
}

static extractpdf_status poster_scan_form_action_nodes(
    fz_context *ctx,
    pdf_obj *fields)
{
    pdf_obj **stack = NULL;
    ...
    stack_capacity = (size_t)pdf_array_len(ctx, fields) + 16;
    if (stack_capacity < 16)
        stack_capacity = 16;
    if (stack_capacity > SIZE_MAX / sizeof(*stack))
        return EXTRACTPDF_ERROR_NOMEM;
    stack = (pdf_obj **)malloc(stack_capacity * sizeof(*stack));
    if (stack == NULL)
        return EXTRACTPDF_ERROR_NOMEM;
    marks = pdf_new_mark_bits(ctx, pdf_get_bound_document(ctx, fields));

    for (i = 0; i < (size_t)pdf_array_len(ctx, fields); ++i)
        stack[stack_count++] = pdf_array_get(ctx, fields, (int)i);

    while (status == EXTRACTPDF_OK && stack_count != 0) {
        pdf_obj *field = stack[--stack_count];
        pdf_obj *kids;
        int kid_count;
        int kid;
        ...
        if (pdf_mark_bits_set(ctx, marks, field)) {
            status = EXTRACTPDF_ERROR_FORMAT;
            break;
        }
        if (poster_has_key(ctx, field, PDF_NAME(A)) ||
            poster_has_key(ctx, field, PDF_NAME(AA))) {
        ...
        if ((size_t)kid_count > SIZE_MAX - stack_count) {
            status = EXTRACTPDF_ERROR_NOMEM;
            break;
        }
        if (stack_count + (size_t)kid_count > stack_capacity) {
            size_t needed = stack_count + (size_t)kid_count;
            ...
            grown = (pdf_obj **)realloc(stack, capacity * sizeof(*stack));
            if (grown == NULL) {
                status = EXTRACTPDF_ERROR_NOMEM;
                break;
            }
        ...
        for (kid = 0; kid < kid_count; ++kid)
            stack[stack_count++] = pdf_array_get(ctx, kids, kid);
    }
...
static extractpdf_status poster_check_acroform_actions(
    fz_context *ctx,
    pdf_document *document)
{
    ...
    if (pdf_dict_get(ctx, form, PDF_NAME(XFA)) != NULL)
        return EXTRACTPDF_ERROR_UNSUPPORTED;
    return poster_scan_form_action_nodes(
        ctx, pdf_dict_get(ctx, form, PDF_NAME(Fields)));
}
    ...
    extractpdf_pdf_poster_plan *plan)
{
    ...
    for (split_index = 0; split_index < plan->split_count; ++split_index) {
        if (!plan->splits[split_index].changed)
            continue;
        status = poster_check_selected_page_keys(ctx, &plan->splits[split_index]);
        if (status != EXTRACTPDF_OK)
            return status;
    }

    status = poster_check_acroform_actions(ctx, document);
    if (status != EXTRACTPDF_OK)
        return status;
    ...
    for (page_index = 0; page_index < page_count; ++page_index) {
        ...
        pdf_obj *annots;
        extractpdf_pdf_poster_split_plan *split = poster_find_split(plan, page_index);
        int annot_count;
        int annot_index;

        if (!pdf_is_dict(ctx, page))
            return EXTRACTPDF_ERROR_FORMAT;
        if (poster_has_key(ctx, page, PDF_NAME(AA)))
            return EXTRACTPDF_ERROR_UNSUPPORTED;
        ...
        for (annot_index = 0; annot_index < annot_count; ++annot_index) {
            ...
            if (is_link) {
                status = poster_check_link_action(ctx, annotation);
                if (status != EXTRACTPDF_OK)
                    return status;
            } else if (poster_has_key(ctx, annotation, PDF_NAME(A)) ||
                       poster_has_key(ctx, annotation, PDF_NAME(AA))) {
                return EXTRACTPDF_ERROR_UNSUPPORTED;
            }
...

            {
                ...
                status = poster_read_public_rect(
                    ctx,
                    annotation,
                    split->page.pdf_to_public,
                ...
                if (is_link) {
                    ...
                    if (pdf_name_eq(ctx, subtype, PDF_NAME(Popup)) ||
                        poster_has_key(ctx, annotation, PDF_NAME(Popup)) ||
                        poster_has_key(ctx, annotation, PDF_NAME(IRT)) ||
                        poster_has_key(ctx, annotation, PDF_NAME(RT)))
                        return EXTRACTPDF_ERROR_UNSUPPORTED;
                ...
                status = extractpdf_pdf_poster_collect_rect_tiles(
                    split,
                    annot_plan->source_public_rect,
                    &annot_plan->tile_indices,
                    &annot_plan->tile_count,
                    &crosses);
                if (status != EXTRACTPDF_OK)
                    return status;
