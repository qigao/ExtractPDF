...
static int public_inside(quantapdf_rect inner, quantapdf_rect outer)
{
    return inner.x0 >= outer.x0 && inner.y0 >= outer.y0 &&
        inner.x1 <= outer.x1 && inner.y1 <= outer.y1;
}

static quantapdf_status read_public_rect(
    fz_context *ctx,
    pdf_obj *annotation,
    ...
    int *out_crosses)
{
    ...
    for (index = 0; index < split->tile_count; ++index) {
        quantapdf_rect tile = split->tiles[index].public_rect;
...
static quantapdf_status check_link_action(fz_context *ctx, pdf_obj *annotation)
{
    ...
    if (dest != NULL && action != NULL)
        return QUANTAPDF_ERROR_FORMAT;
    if (action == NULL)
        return dest != NULL ? QUANTAPDF_OK : QUANTAPDF_ERROR_UNSUPPORTED;
...
static quantapdf_status check_selected_page(
    fz_context *ctx,
...
static quantapdf_status check_form_actions(
    fz_context *ctx,
    pdf_document *document)
{
    ...
    fz_try(ctx)
    {
        ...
        while (status == QUANTAPDF_OK && count != 0) {
            ...
            if (has_key(ctx, node, PDF_NAME(A)) ||
                has_key(ctx, node, PDF_NAME(AA))) {
            ...
            if (count + (size_t)kid_count > capacity) {
                ...
                grown = (pdf_obj **)realloc(
                    stack, grown_capacity * sizeof(*stack));
                if (grown == NULL) {
                    status = QUANTAPDF_ERROR_NOMEM;
                    break;
                }
    ...
    quantapdf_pdf_poster_plan *plan)
{
    ...
    for (page_index = 0; page_index < page_count; ++page_index) {
        ...
        pdf_obj *annots;
        quantapdf_pdf_poster_split_plan *split = find_split(plan, page_index);
        int annot_count;
        int annot_index;
        ...
        if (split != NULL && split->changed && annot_count != 0) {
            if ((size_t)annot_count > SIZE_MAX / sizeof(*split->annots))
                return QUANTAPDF_ERROR_NOMEM;
            split->annots = (quantapdf_pdf_poster_annot_plan *)calloc(
        ...
        for (annot_index = 0; annot_index < annot_count; ++annot_index) {
...

            {
                ...
                if (status != QUANTAPDF_OK)
                    return status;
                if (is_link) {
                    annot_plan->kind = QUANTAPDF_PDF_POSTER_ANNOT_LINK;
                } else if (is_widget) {
                    annot_plan->kind = QUANTAPDF_PDF_POSTER_ANNOT_WIDGET;
                } else {
                    ...
                    if (pdf_name_eq(ctx, subtype, PDF_NAME(Popup)) ||
                        ...
                        return QUANTAPDF_ERROR_UNSUPPORTED;
                    if (!quantapdf_pdf_annotation_classify(ctx, annotation, &type) ||
                        type == QUANTAPDF_ANNOTATION_UNKNOWN)
                        return QUANTAPDF_ERROR_UNSUPPORTED;
                    annot_plan->kind = QUANTAPDF_PDF_POSTER_ANNOT_ORDINARY;
                }
                status = quantapdf_pdf_poster_collect_rect_tiles(
