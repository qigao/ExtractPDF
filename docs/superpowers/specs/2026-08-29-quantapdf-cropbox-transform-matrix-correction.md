# QuantaPDF CropBox V1 Page-Transform Direction Correction

Issue: #49  
PR: #50  
Normative amendment to: `docs/superpowers/specs/2026-08-29-quantapdf-cropbox-transform-design.md`
...
No public API, scope, preservation policy, security policy, or CropBox-only write surface changes. The correction is limited to the internal mapping direction used to convert between raw PDF page coordinates and QuantaPDF's public Fitz page space.
...
```text
quantapdf_crop_pages(document, &full, 1, &noop) == QUANTAPDF_OK
```
...
QuantaPDF's already integrated annotation implementation follows the same contract:

```text
raw PDF /Rect
    -> fz_transform_rect(raw, page_ctm)
    -> public quantapdf_rect
```
...
For QuantaPDF CropBox V1, the matrix returned by:
...
No value returned in `outbox` from `pdf_page_obj_transform()` is used as the public QuantaPDF rectangle. The strict resolver owns the effective-box calculation itself so it does not depend on MuPDF's tolerant repair/fallback behavior.
...
in current QuantaPDF Fitz page space.
