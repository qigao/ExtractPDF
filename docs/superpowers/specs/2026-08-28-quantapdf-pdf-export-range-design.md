# QuantaPDF Contiguous PDF Page Range Export Design
...
Add a thin public helper for exporting one forward contiguous page range to one immutable PDF output while keeping `quantapdf_export_pages(...)` as the only composition engine.
...
```c
quantapdf_export_pages(document, indices, page_count, out_output);
```
...
```text
quantapdf_export_page_range(document, 1, 2, &out)
        |
        v
...
indices = [1, 2]
        |
        v
quantapdf_export_pages(document, indices, 2, &out)
        |
        v
...
3. a versioned `quantapdf_page_range` struct.
...
`first_page` is zero-based, matching `quantapdf_load_page` and `quantapdf_export_pages`.
...
Those cases already belong to the more general `quantapdf_export_pages(...)` index-list primitive.

## Split model

V1 deliberately does not return an array/list of `quantapdf_output` objects.

Per-page split is expressed by repeated single-page calls:

```c
for (int page = 0; page < count; ++page)
    quantapdf_export_page_range(document, page, 1, &output);
```
...
No `quantapdf_output_list`, output array, batch callback, or partial-result collection is introduced.
...
It returns `QUANTAPDF_ERROR_ARGUMENT` for syntactically or arithmetically invalid requests:
...
After safe expansion, it delegates the index list to `quantapdf_export_pages(...)`, which already validates every requested index against the source PDF page count before grafting any page.
...
is rejected by the existing engine with `QUANTAPDF_ERROR_ARGUMENT` and no partial output.
...
For an otherwise mapping-valid request on a successfully opened non-PDF document, delegation reaches `quantapdf_export_pages(...)`, which returns `QUANTAPDF_ERROR_UNSUPPORTED`.
...
After calling `quantapdf_export_pages(...)`, the helper frees the temporary array on both success and failure.

The range helper never owns or retains:

- `quantapdf_document`;
- `quantapdf_output` after returning it;
- MuPDF context/document/output/buffer objects.

The returned `quantapdf_output` has exactly the ownership/lifetime contract already defined by #19 / PR #20: immutable QuantaPDF-owned PDF bytes, independent of the source document after the export call succeeds.

`QUANTAPDF_ERROR_NOMEM` is returned if the temporary index-array allocation fails.
...
All PDF-specific behavior remains in the existing `quantapdf_export_pages(...)` implementation.
...
src/pdf_range.c
    ...
    delegate to quantapdf_export_pages
...
```text
quantapdf_export_page_range(document, 1, 2, &range_output)
```
...
```c
int indices[] = {1, 2};
quantapdf_export_pages(document, indices, 2, &index_output);
```
...
At least the primary `(1, 2)` result is reopened through the existing filename-based `quantapdf_open` test path to verify page count, searchable text, and geometry. Additional cases may use byte equality against equivalent `quantapdf_export_pages` calls to avoid redundant temporary-file machinery.

## Failure cases

Required contract tests include:

1. `document == NULL` -> `QUANTAPDF_ERROR_ARGUMENT`, output reset;
2. `out_output == NULL` -> `QUANTAPDF_ERROR_ARGUMENT`;
3. `first_page < 0` -> `QUANTAPDF_ERROR_ARGUMENT`, output reset;
4. `page_count == 0` -> `QUANTAPDF_ERROR_ARGUMENT`, output reset;
5. `(2, 2)` against the 3-page PDF -> `QUANTAPDF_ERROR_ARGUMENT`, output reset;
6. `first_page = INT_MAX, page_count = 2` -> arithmetic-overflow `QUANTAPDF_ERROR_ARGUMENT` before allocation/delegation;
7. mapping-valid range on `composition-non-pdf.txt` -> `QUANTAPDF_ERROR_UNSUPPORTED`, output reset;
8. successful returned output remains governed by the existing `quantapdf_output_data` / `quantapdf_drop_output` lifecycle.
...
The expected RED is compile failure in only the new test because `quantapdf_export_page_range` is absent from the stacked #20 public header. Existing #20 production code and tests must continue to build.
