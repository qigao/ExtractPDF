# QuantaPDF Phase 5: Typed PDF Document Metadata Design
...
Phase 5 starts with one deliberately small PDF document-root read surface: typed access to the eight standard PDF Info dictionary fields. The API reuses the existing opaque `quantapdf_document`, returns QuantaPDF-owned UTF-8 copies, and keeps all MuPDF PDF object types private.
...
- Establish the first PDF-specific document-root read API on the existing `quantapdf_document` handle.
...
- Define deterministic error/reset behavior consistent with existing QuantaPDF APIs.
...
- a new public `quantapdf_pdf_document` handle.
...
`quantapdf_document_metadata` means **standard PDF Info dictionary metadata only**.

The supported mappings are:

| QuantaPDF field | PDF Info key |
| --- | --- |
| `QUANTAPDF_METADATA_TITLE` | `/Title` |
| `QUANTAPDF_METADATA_AUTHOR` | `/Author` |
| `QUANTAPDF_METADATA_SUBJECT` | `/Subject` |
| `QUANTAPDF_METADATA_KEYWORDS` | `/Keywords` |
| `QUANTAPDF_METADATA_CREATOR` | `/Creator` |
| `QUANTAPDF_METADATA_PRODUCER` | `/Producer` |
| `QUANTAPDF_METADATA_CREATION_DATE` | `/CreationDate` |
| `QUANTAPDF_METADATA_MODIFICATION_DATE` | `/ModDate` |
...
QuantaPDF V1 intentionally distinguishes them, so the implementation must read the PDF Info dictionary directly rather than making `fz_lookup_metadata()` authoritative.

The public API remains independent of this MuPDF implementation detail.

## PDF-only enforcement

The existing `quantapdf_document` remains the only public document handle:
...
pdf_specifics(document->ctx, document->doc)
    ├─ NULL -> QUANTAPDF_ERROR_UNSUPPORTED
    └─ borrowed pdf_document *
```

No extra reference is retained and no second document lifetime is introduced.

A successfully opened non-PDF document is therefore valid for generic QuantaPDF operations but returns `QUANTAPDF_ERROR_UNSUPPORTED` from this PDF-only metadata getter.
...
A non-NULL allocated empty string distinguishes this from absence. The caller releases it with `quantapdf_free()`.

### Present non-empty PDF text string

Decode through MuPDF's PDF text-string conversion to UTF-8, copy into QuantaPDF-owned memory, append a NUL terminator, and return the byte size excluding that terminator.
...
Returned strings follow the existing `quantapdf_extract_text()` ownership convention:

- allocated with QuantaPDF-owned `malloc` memory;
- NUL-terminated;
- `out_size` excludes the NUL;
- caller releases with `quantapdf_free()`;
- returned memory does not retain a document/context/MuPDF object;
- after success the caller may immediately call `quantapdf_close(document)` and continue using the returned string.
...
- `document == NULL` -> `QUANTAPDF_ERROR_ARGUMENT`;
- `out_utf8 == NULL` -> `QUANTAPDF_ERROR_ARGUMENT`;
- `out_size == NULL` -> `QUANTAPDF_ERROR_ARGUMENT`;
- field outside the eight defined enum values -> `QUANTAPDF_ERROR_ARGUMENT`.
...
- malformed non-string selected Info value -> `QUANTAPDF_ERROR_FORMAT`;
- QuantaPDF allocation failure -> `QUANTAPDF_ERROR_NOMEM`;
- MuPDF exceptions -> existing `quantapdf_status_from_mupdf()` mapping;
...
Do not extract a generic `quantapdf_require_pdf_document()` helper in this first read slice. When a later PDF-root feature such as outline or annotations demonstrates the same repeated private gate/error policy, helper extraction can be justified by real duplication.
...
6. Malformed numeric Producer returns `QUANTAPDF_ERROR_FORMAT` and reset outputs.
7. Invalid enum and NULL arguments return `QUANTAPDF_ERROR_ARGUMENT` with reset semantics where output slots exist.
8. Existing non-PDF fixture returns `QUANTAPDF_ERROR_UNSUPPORTED` with reset outputs.
9. Retrieve Title, close the source document, then confirm the copied Title remains valid before `quantapdf_free()`.

A malformed `/Info`-object fixture is not required for V1 because the malformed-object branch is a direct structural validation requirement and introducing a second malformed PDF fixture would enlarge the slice without changing the public contract. The implementation must nevertheless map a present non-dictionary `/Info` to `QUANTAPDF_ERROR_FORMAT`.
...
- only the new metadata contract target fails because `quantapdf_metadata_field` and/or `quantapdf_document_metadata` are absent.
...
- malformed `/Info` structure and malformed selected values map to `QUANTAPDF_ERROR_FORMAT`;
- date values remain raw decoded text;
- returned UTF-8 is independent and freed through `quantapdf_free()`;
