# QuantaPDF Immutable CropBox Transform V1 Design
...
QuantaPDF already has two different PDF operation families:
...
- on success, `out_output` receives a new independent `quantapdf_output`.
...
`sizeof(quantapdf_page_crop)`. Larger values are
...
`quantapdf_page_crop.bounds` is expressed in the **current source page's public Fitz page-space**, the same coordinate system currently returned by:
...
A raw CropBox that extends beyond MediaBox is not itself a format error. An empty/non-positive intersection is `QUANTAPDF_ERROR_FORMAT` because there is no valid visible page region for this transform contract.
...
V1 does not expand or recover content outside the current effective visible region. Any request outside the current effective visible region is `QUANTAPDF_ERROR_ARGUMENT`.
...
Malformed page-tree or box structure returns `QUANTAPDF_ERROR_FORMAT`. Structurally valid page-tree inheritance depth greater than 256 returns `QUANTAPDF_ERROR_UNSUPPORTED`.
...
`quantapdf_crop_pages()` is an atomic immutable batch transform.
...
Duplicate page indices are `QUANTAPDF_ERROR_ARGUMENT`. V1 does not define last-writer-wins semantics.
...
2. `quantapdf_page_box_bounds(..., CROP, ...)` reflects the new visible region;
...
The API still returns a successful immutable output because `quantapdf_crop_pages()` is an output-producing transform primitive.
...
The output is **not** required to be byte-identical to the original input file bytes, because QuantaPDF's serializer may canonicalize a valid source PDF.
...
### `QUANTAPDF_ERROR_MUPDF` / mapped MuPDF statuses
