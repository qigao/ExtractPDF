# QuantaPDF AcroForm Value Mutation V1 Design
...
AcroForm Value Mutation V1 adds deterministic field-value assignment to the already-integrated isolated `quantapdf_pdf_edit` layer.

It deliberately does **not** make the immutable `quantapdf_form` snapshot mutable. Observation and mutation remain separate:

```text
quantapdf_document
    -> quantapdf_document_form()
    -> immutable quantapdf_form
...
quantapdf_pdf_edit
    ...
    -> session-local quantapdf_form_field_ref
...
current editor PDF
      ...
      |      |
      |      +-- immutable semantic model -> quantapdf_form
      |      |
...
The public `quantapdf_form` remains deep-owned, document-independent, and free of MuPDF pointers.
...
A form field ref identifies one **logical terminal field group** inside one `quantapdf_pdf_edit` session.
...
The internal form token must incorporate a form-specific domain salt in addition to the editor session cookie, slot, and tag. A bitwise-copied annotation ref passed to a form API must fail validation as `QUANTAPDF_ERROR_ARGUMENT`.
...
```c
typedef struct quantapdf_pdf_edit_form_locator {
    size_t step_count;
    uint32_t *steps;
} quantapdf_pdf_edit_form_locator;

typedef struct quantapdf_pdf_edit_form_entry {
    quantapdf_pdf_edit_form_locator locator;
    uint32_t tag;
} quantapdf_pdf_edit_form_entry;
```
...
If a syntactically valid session ref cannot resolve its locator to exactly one current logical terminal field after a successful strict parse, the editor returns `QUANTAPDF_ERROR_STATE`. It must not guess by field name, scan for an equivalent value, silently register a replacement identity, or repair the registry after the fact.
...
V1 does not create/delete/reparent fields, so form refs have no public tombstone lifecycle. The namespace ends when `quantapdf_drop_pdf_edit()` is called.
...
`quantapdf_pdf_edit_form_snapshot()` runs the same strict semantic parser used by `quantapdf_document_form()` against the editor's current private PDF state and publishes the same immutable `quantapdf_form` type.
...
- survives `quantapdf_drop_pdf_edit()`;
...
`quantapdf_pdf_edit_form_field_ref_at()` re-runs the current strict parser with an optional private provenance sidecar.
...
This V1 therefore includes a targeted refactor of the already-integrated `quantapdf_pdf_form_reconcile_widgets()` implementation so both:

```text
quantapdf_document_form()
quantapdf_pdf_edit_form_snapshot() / field_ref_at()
```
...
`quantapdf_pdf_edit_form_snapshot()` and `quantapdf_pdf_edit_form_field_ref_at()` continue to use normal AcroForm Snapshot V1 read semantics.

The mutation-only restrictions on `/XFA` and `/NeedAppearances` apply only to `quantapdf_pdf_edit_form_set_values()`.
...
```c
offsetof(quantapdf_form_value_input, utf8_size)
...
`quantapdf_form_value_update` is passed as one structure, so too-small values
return `ARGUMENT` while current-size and larger values are accepted and unknown
trailing bytes are ignored.

`quantapdf_form_value_input` is traversed through `update.values` as a C array
without explicit stride. Each V1 input must therefore use
`struct_size` covering through `utf8_size` and no larger than
`sizeof(quantapdf_form_value_input)`; larger values are `ARGUMENT`. Future
input-element growth requires a new type/API or an explicit-stride API.
...
The setter writes normalized raw PDF semantic state directly according to the already-validated QuantaPDF model.
...
This targeted raw-page refactor is part of #46 because it is required to make editor form observation and mutation preflight genuinely side-effect-free. Existing `quantapdf.pdf_form` tests remain mandatory regressions.
...
No `pdf_load_page()` or page-wide update is permitted **after** `/V` or `/I` has been changed. QuantaPDF code never directly invokes `pdf_update_page()` or `pdf_update_open_pages()`.
...
prepare QuantaPDF-owned allocations
...
Workflow #266 strengthens the contract: **journal abandon is necessary but not sufficient for public API atomicity**. MuPDF's journal owns PDF-object mutations, but it does not automatically make QuantaPDF's C registries, retained object pointers, page wrappers, Widget editing flags, or resynthesis bookkeeping transactionally valid.
...
- `quantapdf_pdf_edit_form_snapshot()` observes the same semantic form state as before the call;
- a newly produced deterministic `quantapdf_pdf_edit_snapshot()` has exactly the same size and bytes as a snapshot taken immediately before the failed call, when no intervening successful mutation occurred;
...
- no page/Widget wrapper, editing state, resynthesis flag, or stale live object retained by QuantaPDF can affect a later public operation;
...
`quantapdf_pdf_edit_form_snapshot()` resets `*out_form = NULL` before later validation.

`quantapdf_pdf_edit_form_field_ref_at()` zeroes both opaque words before later validation.

`quantapdf_pdf_edit_form_set_values()` has no output object to publish and never partially changes caller-owned inputs.
...
Read-only AcroForm tests remain in `quantapdf.pdf_form`; mutation tests do not enlarge that target.

The existing read-only Form target must also gain or reuse a deterministic `/NeedAppearances true` fixture proving that `quantapdf_document_form()` is byte-preserving after the raw-page reconciliation refactor.
...
begin editor
   ...
   -> pdf_edit_snapshot
   -> open output as new quantapdf_document
   -> quantapdf_document_form
...
Windows evidence must show that `quantapdf.dll` and `quantapdf_test_pdf_form_mutation.exe` are actually built and that `quantapdf.pdf_form_mutation` runs as test 21/21.
