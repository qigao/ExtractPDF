# ExtractPDF Immutable Poster Split V1 Design
...
That behavior is correct for ExtractPDF's Phase 4 composition APIs but insufficient for Poster Split, whose public contract explicitly covers links, annotations, Widgets, AcroForm relationships, outlines, and internal destinations.
...
```c
struct_size >= offsetof(extractpdf_page_poster_split, rows)
             + sizeof(size_t)
```

Smaller values return `EXTRACTPDF_ERROR_ARGUMENT`.

Larger values are accepted for ABI-forward compatibility.
...
PDF user space --pdf_to_public--> ExtractPDF/Fitz public page space
```
...
The implementation must reject the request as `EXTRACTPDF_ERROR_ARGUMENT` if finite source geometry plus requested grid density cannot produce strictly positive-area public tiles under the implementation's float representation.
...
Duplicate `page_index` requests are `EXTRACTPDF_ERROR_ARGUMENT` even if they specify identical grids.
...
Final page count above `INT_MAX` is `EXTRACTPDF_ERROR_ARGUMENT`.
...
If canonical source serialization changes any fact required to reproduce the source split plan, the changed path returns `EXTRACTPDF_ERROR_FORMAT` before its first page-graph mutation.

## 8. Security policy

V1 keeps the same fail-closed rewrite security model as CropBox and MediaBox Trim:

- non-PDF input: `EXTRACTPDF_ERROR_UNSUPPORTED`;
- encrypted PDF: `EXTRACTPDF_ERROR_UNSUPPORTED`;
- already-signed PDF: `EXTRACTPDF_ERROR_UNSUPPORTED`;
...
For a non-no-op split, the selected source page is `EXTRACTPDF_ERROR_UNSUPPORTED` if it has/effectively supplies any of:
...
After accounting for standard structural keys, the safe-share allowlist, the explicit page-box inputs, and `/Annots`, an unrecognized extra key on a selected source page is `EXTRACTPDF_ERROR_UNSUPPORTED` for a real expansion.
...
Other Link action kinds are `EXTRACTPDF_ERROR_UNSUPPORTED` for a real split.

Selected-page Link `/AA` is `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
A source Link whose rectangle has no positive-area intersection with the source visible split domain is `EXTRACTPDF_ERROR_UNSUPPORTED`; V1 does not silently drop an invisible/out-of-domain Link object.
...
- its subtype is recognized by ExtractPDF's existing annotation model or explicitly accepted by Poster Split preflight;
...
If the rectangle crosses a tile boundary, return `EXTRACTPDF_ERROR_UNSUPPORTED`.

If the rectangle has no containing tile because it lies entirely outside the source visible split domain, return `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
A Widget crossing a tile boundary returns `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
- `/Fit`, `/FitB`, `/FitH`, `/FitBH`, `/FitV`, `/FitBV`, `/FitR`, or other destination kinds are `EXTRACTPDF_ERROR_UNSUPPORTED` in V1;
- `/XYZ` with missing/null x or y is `EXTRACTPDF_ERROR_UNSUPPORTED` because the tile cannot be selected uniquely by the V1 rule;
- malformed destination arrays/types are `EXTRACTPDF_ERROR_FORMAT`.
...
If the `/XYZ` point lies outside the source effective visible split domain, the transform returns `EXTRACTPDF_ERROR_UNSUPPORTED` rather than clamping it to a tile.
...
Similarly, annotation/Widget action containers outside the supported Link URI/GoTo model are `EXTRACTPDF_ERROR_UNSUPPORTED` when they occur on a selected page whose object must move.
...
If the catalog contains `/StructTreeRoot`, any batch containing a real split (`columns * rows > 1`) returns `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
A real split is `EXTRACTPDF_ERROR_UNSUPPORTED` when the selected source page has an effective/local/inherited:
...
If preflight cannot prove this invariant for a selected-page Widget, the real split is `EXTRACTPDF_ERROR_UNSUPPORTED` or `EXTRACTPDF_ERROR_FORMAT` according to whether the source is valid-but-out-of-scope or malformed.
...
The new `extractpdf.pdf_poster_split` CTest must lock at least these behaviors.
...
Returns `EXTRACTPDF_ERROR_UNSUPPORTED` with `*out_output == NULL`.
...
Returns `EXTRACTPDF_ERROR_UNSUPPORTED`.
