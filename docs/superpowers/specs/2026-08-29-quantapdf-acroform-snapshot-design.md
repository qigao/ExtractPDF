# QuantaPDF AcroForm Snapshot V1 Design
...
A later Form Value Mutation V1 will reuse the same strict semantic parser inside the already-integrated `quantapdf_pdf_edit`. That later mutation work is not part of #43.
...
- snapshot data remains valid until `quantapdf_drop_form()`;
- `quantapdf_drop_form(NULL)` is a no-op;
...
`quantapdf_form_field_label()` exposes effective alternate field name `/TU` resolved through the validated group/parent graph.
...
For `quantapdf_form_value_info`, UTF8 resets `option_index` to `SIZE_MAX`.
...
`quantapdf_document_form()` is pure observation at the QuantaPDF contract level.
...
Encrypted PDFs are observed only through the existing authenticated `quantapdf_open()` source handle; Snapshot V1 adds no new encryption policy.
