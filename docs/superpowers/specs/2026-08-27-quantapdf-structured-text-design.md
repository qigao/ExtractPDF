# QuantaPDF Structured Text Snapshot Design
...
## Why an QuantaPDF-owned snapshot
...
The V1 API instead copies the relevant content into one opaque immutable `quantapdf_text_page` snapshot. Public traversal is index-based, which is straightforward for C and .NET/PInvoke and leaves storage/layout free to change internally.
...
`quantapdf.h` therefore adds `<stdint.h>` but no MuPDF header or type.

### C identifier rule

C typedef names and function names share the ordinary identifier namespace. Therefore the accessor functions deliberately use `get_*_info` names rather than colliding with the `quantapdf_text_*_info` typedefs.
...
This append-only rule applies to the single `*_info` objects passed to getters.
`quantapdf_search_result` is different because search traverses a caller-owned
C array without an explicit stride. Search-result elements have a fixed V1
layout and require `struct_size` to cover through `quad` without exceeding
`sizeof(quantapdf_search_result)`; extending them requires a new type/API or
an explicit-stride search API.
...
- `quantapdf_extract_structured_text` sets `*out_text = NULL` on every failure.
- A successful snapshot owns all arrays and strings it exposes and no longer depends on `quantapdf_page`, `quantapdf_document`, `fz_context`, or any MuPDF object.
- The source page and document may be dropped immediately after extraction.
- `quantapdf_text_span_text` returns a borrowed NUL-terminated UTF-8 pointer owned by the snapshot. `out_size` excludes the trailing NUL.
- Borrowed span pointers remain valid until `quantapdf_drop_text_page`.
- `quantapdf_drop_text_page(NULL)` is safe.
...
Block order and line order follow MuPDF's extracted structured-text order. QuantaPDF does not claim to reconstruct a higher-level semantic reading order beyond what MuPDF returns.
...
QuantaPDF forms a new span whenever consecutive characters differ in style identity. V1 style identity consists of:
...
`src/structured_text.c` owns the projection from MuPDF into QuantaPDF storage. The opaque snapshot contains compact arrays conceptually equivalent to:
...
No internal pointer in the snapshot points back into MuPDF memory after `quantapdf_extract_structured_text` returns.

## Error handling

- NULL handles/output pointers, undersized `struct_size`, and out-of-range indices return `QUANTAPDF_ERROR_ARGUMENT`.
- Count outputs are reset to `0` on failure when supplied.
- Span text outputs are reset to `NULL` / `0` on failure when supplied.
- Allocation/overflow failures return `QUANTAPDF_ERROR_NOMEM` and unwind all partially built arrays/strings.
