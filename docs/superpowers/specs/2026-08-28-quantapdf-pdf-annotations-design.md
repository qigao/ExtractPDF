# QuantaPDF PDF Page Annotation Enumeration V1 Design
...
- Reuse the existing opaque `quantapdf_page`.
- Return an QuantaPDF-owned immutable annotation snapshot with no MuPDF/PDF pointer in the public ABI.
...
`quantapdf_annotation_type` is an QuantaPDF enum, not a cast of MuPDF's enum. Its numeric values belong to the public ABI.
...
```text
type      stable QuantaPDF type; UNKNOWN for missing/non-name/unrecognized subtype
...
- malformed surviving data -> `QUANTAPDF_ERROR_FORMAT` for the whole extraction.
...
PDF annotation `/Rect` is in PDF page user space. QuantaPDF maps it to the same Fitz page-space model used by links/text/images/page bounds.
...
```text
quantapdf_extract_annotations() -> QUANTAPDF_OK
*out_annotations                 -> non-NULL snapshot
quantapdf_annotation_count()    -> 0
```
...
The published snapshot contains only QuantaPDF-owned item records and copied strings. It retains no `pdf_annot *`, `pdf_obj *`, `pdf_page *`, `fz_page *`, `fz_document *`, `quantapdf_page *`, or `quantapdf_document *`.

After successful extraction callers may immediately drop the source page and close the document. Snapshot accessors remain valid until `quantapdf_drop_annotation_page()`; dropping NULL is a no-op.
...
Accessors reset caller-visible outputs to neutral values before later handle/index validation wherever possible. `quantapdf_annotation_get_info()` validates `struct_size` first, then resets known fields before handle/index validation.
...
Final exact head must pass Linux strict static/all CTests, Linux ASan/UBSan/all CTests, macOS configure/build/test, and Windows DLL configure/build/test with `quantapdf.pdf_annotations` executed on Windows.
