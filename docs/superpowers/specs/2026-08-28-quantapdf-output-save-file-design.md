# QuantaPDF Immutable Output File Save Design
...
QuantaPDF already materializes every composition result as an immutable, QuantaPDF-owned `quantapdf_output` byte snapshot. V1 adds one optional terminal adapter that writes those already-materialized bytes to a filesystem path:
...
immutable quantapdf_output
        |
        +--> quantapdf_output_data(...)      borrowed bytes
        |
        +--> quantapdf_output_save_file(...) optional terminal adapter
        |
        v
...
V1 chooses a **file-save adapter on `quantapdf_output`** rather than a callback sink or a new composition-to-file path.
...
```c
quantapdf_output_save_file(output, filename);
```

Benefits:

- composition remains memory-backed and filename-free;
- the adapter consumes the exact immutable bytes already exposed by `quantapdf_output_data(...)`;
...
Callers can already use `quantapdf_output_data(...)` and perform their own file I/O. Keeping only that surface would preserve the smallest possible ABI, but the library would still lack a common UTF-8 filesystem/error contract for the Phase 4 `Save/write` roadmap item.
...
It belongs beside the existing `quantapdf_output_data(...)` lifecycle surface.
...
`quantapdf_output_save_file(...)` is **not** a PDF composition primitive and is **not** a PDF serialization primitive.

By the time it is called, `quantapdf_output` already contains the complete final PDF file image:

```text
struct quantapdf_output   // private
    unsigned char *data
    size_t size
```
...
quantapdf_output_save_file(output, "first.pdf");
quantapdf_output_save_file(output, "second.pdf");
quantapdf_output_data(output, &data, &size);
quantapdf_output_save_file(output, "third.pdf");
quantapdf_drop_output(output);
```

The caller still owns the same `quantapdf_output` until `quantapdf_drop_output(...)`.

`quantapdf_output_save_file(...)` never:
...
A failed save leaves the immutable output usable for another save, `quantapdf_output_data(...)`, or final drop.

## Argument contract

Return `QUANTAPDF_ERROR_ARGUMENT` when:
...
A non-NULL `quantapdf_output *` is treated according to the existing opaque-handle invariant. V1 does not expose private output fields or add a second public output-validity contract.

## UTF-8 path contract

All public QuantaPDF paths are UTF-8. The save adapter must preserve that contract explicitly on every supported platform.
...
If strict UTF-8 conversion fails, return `QUANTAPDF_ERROR_ARGUMENT`.

If allocating the UTF-16 path buffer fails, return `QUANTAPDF_ERROR_NOMEM`.
...
No locale-dependent conversion layer is introduced by QuantaPDF.
...
If the parent path does not exist or the target cannot be opened for writing, return `QUANTAPDF_ERROR_IO`.
...
A zero-byte write before the requested total is reached is an I/O failure. The implementation may consult `ferror(file)` for diagnostics/control, but the public status remains `QUANTAPDF_ERROR_IO`.
...
`QUANTAPDF_OK` is returned only after all three stages succeed:
...
If exact writing succeeds but `fflush` fails, return `QUANTAPDF_ERROR_IO`.

The opened stream must still be closed after a write or flush failure. A save that already has an I/O failure remains `QUANTAPDF_ERROR_IO` regardless of the close result.

If writing and flushing succeed but `fclose` reports failure, return `QUANTAPDF_ERROR_IO`.

This makes close-time writeback errors observable rather than silently treating them as success.

## What `QUANTAPDF_OK` does not mean
...
This behavior is explicit so callers requiring atomic-replace semantics can implement an application-level temporary-file/rename policy without QuantaPDF pretending to provide guarantees it does not actually enforce.

The input `quantapdf_output`, however, remains unchanged and reusable regardless of filesystem failure.
...
```text
output == NULL                         -> QUANTAPDF_ERROR_ARGUMENT
filename == NULL                       -> QUANTAPDF_ERROR_ARGUMENT
filename == ""                         -> QUANTAPDF_ERROR_ARGUMENT
invalid UTF-8 path on Windows          -> QUANTAPDF_ERROR_ARGUMENT
Windows UTF-16 path allocation failure -> QUANTAPDF_ERROR_NOMEM
file open failure                      -> QUANTAPDF_ERROR_IO
short/failed write that cannot finish  -> QUANTAPDF_ERROR_IO
fflush failure                         -> QUANTAPDF_ERROR_IO
fclose failure                         -> QUANTAPDF_ERROR_IO
otherwise                              -> QUANTAPDF_OK
```
...
Those conditions either succeed according to the host or return `QUANTAPDF_ERROR_IO` through the ordinary open/write/flush/close path.
...
include/quantapdf/quantapdf.h
    add quantapdf_output_save_file declaration
...
The private `quantapdf_output` definition remains exactly the current `data + size` snapshot.
...
No public Windows types appear in `quantapdf.h`.
...
```c
int indices[] = {2, 0};
quantapdf_export_pages(document, indices, 2, &output);
```
...
```c
quantapdf_output_save_file(output, ASCII_OUTPUT_PDF);
```
...
on-disk bytes == quantapdf_output_data(...) bytes byte-for-byte
```
...
Reopen the saved file through `quantapdf_open(...)` and verify:
...
```c
quantapdf_output_save_file(output, UTF8_OUTPUT_PDF);
```
...
```c
quantapdf_open(UTF8_OUTPUT_PDF, NULL, &saved_document);
```

Then verify the same two pages and geometry.

On MSVC, compile the new test target with `/utf-8`, matching the existing `quantapdf.document` UTF-8 path test precedent.
...
```text
<build>/quantapdf-save-missing-parent/output.pdf
```
...
```c
quantapdf_output_save_file(output, MISSING_PARENT_PDF);
```
...
```c
quantapdf_output_save_file(output, invalid_utf8);
```
...
Finally drop the output exactly once through `quantapdf_drop_output(...)`.
...
```text
quantapdf_output_save_file(NULL, valid_path)
    -> QUANTAPDF_ERROR_ARGUMENT

quantapdf_output_save_file(output, NULL)
    -> QUANTAPDF_ERROR_ARGUMENT

quantapdf_output_save_file(output, "")
    -> QUANTAPDF_ERROR_ARGUMENT
```
...
- only `quantapdf_test_output_file` fails because `quantapdf_output_save_file` is absent;
...
- the new function is a public `QUANTAPDF_API` DLL export;
