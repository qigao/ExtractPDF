# ExtractPDF Poster Split V1 Destination / Action Preflight Correction

Issue: #53  
Normative amendment to: `docs/superpowers/specs/2026-08-30-extractpdf-poster-split-design.md`  
...
Poster Split defines row/column order in source ExtractPDF/Fitz public page space. Under Rotate 90 or 270, public row/column boundaries map to different raw PDF axes/directions. A rule such as "the final raw row includes the final edge" is therefore not a stable definition of visual row-major ownership.
...
If the transformed public destination point lies outside `visible_public`, the real split is `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
- malformed arrays/entries that prevent classification are `EXTRACTPDF_ERROR_FORMAT`.
...
For any real split, an annotation that is not a Link and contains `/A` or `/AA` anywhere in the document is `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
- catalog `/AcroForm/XFA` present -> `EXTRACTPDF_ERROR_UNSUPPORTED`;
- any AcroForm field dictionary with `/A` or `/AA` -> `EXTRACTPDF_ERROR_UNSUPPORTED`;
- any Widget with `/A` or `/AA` -> `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
Other action kinds or `/Next` action chains are `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
The final `extractpdf.pdf_poster_split` characterization must explicitly add:
