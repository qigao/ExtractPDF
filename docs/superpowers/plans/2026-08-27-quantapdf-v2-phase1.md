# QuantaPDF v2 Phase 1 implementation plan
...
**Canonical architecture:** every supported desktop platform uses the same pinned vcpkg baseline and the repository's MuPDF 1.28.2 overlay port. MuPDF stays private to QuantaPDF. Windows links static MuPDF into `quantapdf.dll`; Linux/macOS build QuantaPDF static in CI.

**Spec:** `docs/superpowers/specs/2026-08-27-quantapdf-v2-design.md`
...
- No mutable process-global or thread-local document state in QuantaPDF-owned code.
...
- QuantaPDF-owned C code treats warnings as errors.
...
- [x] Add opaque `quantapdf_document` public handle.
- [x] Lock status enum values `0..7` and immutable status strings.
- [x] Export `quantapdf_open`, `quantapdf_page_count`, `quantapdf_status_string`, and `quantapdf_close`.
- [x] Add Windows export/import macro scoped to QuantaPDF only.
...
- [x] Map allocation/context failure to `QUANTAPDF_ERROR_NOMEM`.
...
- [x] Keep `quantapdf_close(NULL)` safe.
...
- [x] Create a configured UTF-8 fixture path (`quantapdf-测试.pdf`).
- [x] Compile the MSVC UTF-8 path test with `/utf-8`.
- [x] Build `quantapdf.dll` on Windows while keeping MuPDF private/static.
- [x] Stage `quantapdf.dll` beside each Windows test executable so loader behavior is deterministic.
...
- [x] Linux bootstraps pinned vcpkg, installs overlay MuPDF 1.28.2, builds/tests static QuantaPDF, then runs ASan/UBSan.
- [x] macOS bootstraps the same vcpkg baseline, selects `arm64-osx` or `x64-osx`, and builds/tests static QuantaPDF.
- [x] Windows bootstraps the same baseline, installs `x64-windows-static-md`, builds `quantapdf.dll`, and runs CTest.
...
- [x] No mutable QuantaPDF-owned global/TLS document state.
...
- [x] Windows architecture is static MuPDF privately linked into shared QuantaPDF.
