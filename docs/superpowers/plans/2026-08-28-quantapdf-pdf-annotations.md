# QuantaPDF PDF Page Annotation Enumeration V1 Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-08-28-quantapdf-pdf-annotations-design.md`

## Locked constraints

- Reuse `quantapdf_page`; no second PDF page handle.
...
- [x] Register `quantapdf.pdf_annotations` in CMake, including Windows DLL-copy target list.
...
The library and every pre-existing target must build. Only the new annotation test target may fail, and it must fail because `quantapdf_annotation_page`, enum/info types, constants, and annotation APIs do not yet exist. Runtime/fixture errors are not valid RED.
...
typedef enum quantapdf_annotation_type { ... } quantapdf_annotation_type;
...
quantapdf_status quantapdf_extract_annotations(
    quantapdf_page *, quantapdf_annotation_page **);
quantapdf_status quantapdf_annotation_count(
    const quantapdf_annotation_page *, size_t *);
quantapdf_status quantapdf_annotation_get_info(
    const quantapdf_annotation_page *, size_t, quantapdf_annotation_info *);
quantapdf_status quantapdf_annotation_contents(
    const quantapdf_annotation_page *, size_t, const char **, size_t *);
void quantapdf_drop_annotation_page(quantapdf_annotation_page *);
```
...
Use existing `quantapdf_status_from_mupdf()` for MuPDF exceptions. Allocation/size overflow -> `NOMEM`.
...
Windows must execute `quantapdf.pdf_annotations`, proving export/link/runtime behavior through the shared-library build.
