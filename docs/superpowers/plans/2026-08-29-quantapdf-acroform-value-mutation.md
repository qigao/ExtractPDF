# QuantaPDF AcroForm Value Mutation V1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add deterministic, session-local, journal-atomic AcroForm value mutation to the existing `quantapdf_pdf_edit` layer without making `quantapdf_form` mutable or executing PDF form runtime behavior.
...
**Spec:** `docs/superpowers/specs/2026-08-29-quantapdf-acroform-value-mutation-design.md`
...
- Existing suite starts at **20 CTests**. This slice adds exactly one new public test target, `quantapdf.pdf_form_mutation`, for **21 total**.
...
- `quantapdf_form` remains immutable, deep-owned, document-independent, and free of MuPDF pointers.
- Mutation exists only on `quantapdf_pdf_edit`; do not introduce a second mutable form/document handle.
- Public mutation identity is `quantapdf_form_field_ref`. Snapshot indices, field names, Widget indices, PDF object numbers, and option strings are never public mutation identity.
- Form refs use a token domain distinct from annotation refs and are valid only for one editor session.
- `quantapdf_document_form()`, `quantapdf_pdf_edit_form_snapshot()`, `quantapdf_pdf_edit_form_field_ref_at()`, and strict form parsing/reconciliation must not call `pdf_load_page()`, `fz_load_page()`, `pdf_update_page()`, `pdf_update_open_pages()`, or form event/runtime APIs.
- Raw page reconciliation uses `pdf_lookup_page_obj()`, raw `/Annots`, and `pdf_page_obj_transform()` exactly as locked by the spec.
- Value mutation V1 supports Text, Checkbox, Radio, Combo, and List. PushButton, Signature, UNKNOWN, Text RichText, and Text FileSelect are `QUANTAPDF_ERROR_UNSUPPORTED` after command validation.
...
- `include/quantapdf/quantapdf.h` — approved ref/value-update ABI and exactly three editor form APIs.
...
- `src/pdf_form.c` — reusable private PDF-to-`quantapdf_form` snapshot builder.
...
**Interfaces:**
- Consumes: existing `quantapdf_pdf_edit`, immutable `quantapdf_form`, current 20-test suite.
...
```cmake
add_executable(quantapdf_test_pdf_form_mutation
  test_pdf_form_mutation.c)
target_link_libraries(quantapdf_test_pdf_form_mutation PRIVATE QuantaPDF::QuantaPDF)
target_compile_definitions(quantapdf_test_pdf_form_mutation PRIVATE
...
add_test(NAME quantapdf.pdf_form_mutation
  COMMAND quantapdf_test_pdf_form_mutation)
set_tests_properties(quantapdf.pdf_form_mutation PROPERTIES TIMEOUT 60)
```
...
new quantapdf_test_pdf_form_mutation fails compilation
compiler errors name quantapdf_form_field_ref / value input/update / three absent functions
```
...
**Interfaces:**
- Consumes: current immutable `quantapdf_document_form()`.
...
Add this fixture define to the existing `quantapdf_test_pdf_form` target:
...
```c
static quantapdf_output *snapshot_fresh_document(const char *path, int observe_form)
{
    quantapdf_document *d = NULL;
    quantapdf_pdf_edit *edit = NULL;
    quantapdf_output *out = NULL;
    quantapdf_form *form = NULL;

    CHECK(quantapdf_open(path, NULL, &d) == QUANTAPDF_OK);
    if (observe_form) {
        CHECK(quantapdf_document_form(d, &form) == QUANTAPDF_OK);
        CHECK(form != NULL);
        quantapdf_drop_form(form);
    }
    CHECK(quantapdf_pdf_edit_begin(d, &edit) == QUANTAPDF_OK);
    CHECK(quantapdf_pdf_edit_snapshot(edit, &out) == QUANTAPDF_OK);
...
{
    quantapdf_output *before = snapshot_fresh_document(
        ACROFORM_NEED_APPEARANCES_PDF, 0);
    quantapdf_output *after = snapshot_fresh_document(
    ...
    CHECK(quantapdf_output_data(before, &a, &na) == QUANTAPDF_OK);
    CHECK(quantapdf_output_data(after, &b, &nb) == QUANTAPDF_OK);
    CHECK(na == nb);
    CHECK(memcmp(a, b, na) == 0);
    quantapdf_drop_output(before);
    quantapdf_drop_output(after);
}
```
...
Required pre-change result: at least one match inside `quantapdf_pdf_form_reconcile_widgets()`.
...
ctest --test-dir build -R '^quantapdf\.pdf_form$' --output-on-failure
```
...
```c
for (page_index = 0;
     page_index < page_count && status == QUANTAPDF_OK;
     ++page_index) {
    ...
    for (ai = 0; ai < acount && status == QUANTAPDF_OK; ++ai) {
        ...
        status = quantapdf_pdf_read_rect_strict(
            ctx, obj, PDF_NAME(Rect), page_ctm, &widget.bounds);
        if (status == QUANTAPDF_OK)
            status = quantapdf_pdf_read_u32_default(
...
cmake --build build --target \
  quantapdf_test_pdf_annotations \
  quantapdf_test_pdf_annotation_mutation \
  quantapdf_test_pdf_form --parallel 2
ctest --test-dir build -R 'quantapdf\.(pdf_annotations|pdf_annotation_mutation|pdf_form)$' --output-on-failure
```
...
- Private output: `quantapdf_pdf_form_provenance`, reusable PDF-to-form builder, form-ref register/resolve functions.
...
static void test_editor_snapshot_and_refs(void)
{
    quantapdf_document *d = NULL;
    quantapdf_pdf_edit *edit = NULL;
    quantapdf_form *a = NULL;
    quantapdf_form *b = NULL;
    quantapdf_form_field_ref r0 = {{9, 9}};
    quantapdf_form_field_ref r0b = {{8, 8}};
    quantapdf_form_field_ref r1 = {{7, 7}};
    size_t count = 0;

    CHECK(quantapdf_open(FORM_MUTATION_BASIC_PDF, NULL, &d) == QUANTAPDF_OK);
    CHECK(quantapdf_pdf_edit_begin(d, &edit) == QUANTAPDF_OK);
    quantapdf_close(d);

    CHECK(quantapdf_pdf_edit_form_snapshot(edit, &a) == QUANTAPDF_OK);
    CHECK(quantapdf_form_field_count(a, &count) == QUANTAPDF_OK);
    CHECK(count == 4);

    CHECK(quantapdf_pdf_edit_form_field_ref_at(edit, 0, &r0) == QUANTAPDF_OK);
    CHECK(quantapdf_pdf_edit_form_field_ref_at(edit, 0, &r0b) == QUANTAPDF_OK);
    CHECK(memcmp(&r0, &r0b, sizeof(r0)) == 0);
    CHECK(quantapdf_pdf_edit_form_field_ref_at(edit, 1, &r1) == QUANTAPDF_OK);
    CHECK(memcmp(&r0, &r1, sizeof(r0)) != 0);

    CHECK(quantapdf_pdf_edit_form_snapshot(edit, &b) == QUANTAPDF_OK);
    quantapdf_drop_pdf_edit(edit);

    CHECK(quantapdf_form_field_count(a, &count) == QUANTAPDF_OK);
    CHECK(count == 4);
    CHECK(quantapdf_form_field_count(b, &count) == QUANTAPDF_OK);
    CHECK(count == 4);
    quantapdf_drop_form(a);
    quantapdf_drop_form(b);
}
```
...
```c
CHECK(quantapdf_pdf_edit_form_snapshot(NULL, &a) == QUANTAPDF_ERROR_ARGUMENT);
CHECK(a == NULL);
memset(&r0, 0xA5, sizeof(r0));
CHECK(quantapdf_pdf_edit_form_field_ref_at(edit, SIZE_MAX, &r0) ==
...
quantapdf_status quantapdf_pdf_form_snapshot_from_pdf(
    fz_context *ctx,
    pdf_document *document,
    quantapdf_form **out_form);
```
...
quantapdf_pdf_form_live_widget *grown = realloc(
    live->widgets,
...
`quantapdf_pdf_form_snapshot_from_pdf()` must:

```text
reset *out_form = NULL
call quantapdf_pdf_form_build(ctx, document, 0, &model, NULL)
allocate quantapdf_form
attach model
publish only after all steps succeed
```

Make `quantapdf_document_form()` validate/obtain `pdf_document *` and delegate to it. `quantapdf_pdf_edit_form_snapshot()` calls the same helper against `edit->document`.
...
Tag derivation must include `QUANTAPDF_FORM_REF_DOMAIN`, so a bitwise annotation ref fails form-token validation even when its slot number matches.
...
if (edit == NULL || edit->ctx == NULL || edit->document == NULL)
    return QUANTAPDF_ERROR_ARGUMENT;
return quantapdf_pdf_form_snapshot_from_pdf(
...
```bash
cmake --build build --target quantapdf_test_pdf_form_mutation --parallel 2
ctest --test-dir build -R '^quantapdf\.pdf_form_mutation$' --output-on-failure
```
...
```bash
git add include/quantapdf/quantapdf.h CMakeLists.txt \
    ...
    size_t *out_size)
{
    ...
    *out_size = 0;
    CHECK(quantapdf_pdf_edit_snapshot(edit, &out) == QUANTAPDF_OK);
    CHECK(quantapdf_output_data(out, &data, &size) == QUANTAPDF_OK);
    CHECK(size != 0);
    *out_data = malloc(size);
    CHECK(*out_data != NULL);
    memcpy(*out_data, data, size);
    *out_size = size;
    quantapdf_drop_output(out);
}

static quantapdf_form_field_ref field_ref_by_name(
    quantapdf_pdf_edit *edit,
    const char *wanted)
{
    quantapdf_form *form = NULL;
    ...
    for (i = 0; i < count; ++i) {
        ...
        size_t size = 0;
        CHECK(quantapdf_form_field_name(form, i, &name, &size) == QUANTAPDF_OK);
        if (name != NULL && size == strlen(wanted) &&
            memcmp(name, wanted, size) == 0) {
            CHECK(quantapdf_pdf_edit_form_field_ref_at(edit, i, &ref) == QUANTAPDF_OK);
            quantapdf_drop_form(form);
            return ref;
        }
    }
...
static void test_widget_prepare_is_byte_preserving(void)
{
    ...
    quantapdf_test_pdf_edit_set_fault(
        edit,
        QUANTAPDF_TEST_PDF_EDIT_FAULT_FORM_AFTER_WIDGET_PREPARE);
...
Require every target Widget to resolve exactly once. Drop every owned page in `quantapdf_pdf_edit_form_drop_widget_handles()`.
...
if (edit->test_fault ==
    QUANTAPDF_PDF_EDIT_TEST_FAULT_FORM_AFTER_WIDGET_PREPARE) {
...
Without the injected fault, return `QUANTAPDF_ERROR_UNSUPPORTED` in Task 4; successful mutation begins in Task 5.
...
**Interfaces:**
- Produces `quantapdf_pdf_edit_form_assignment` plus validate/equal/write helpers.
...
Primary oracle for every success: editor form snapshot, `pdf_edit_snapshot()`, reopen, `quantapdf_document_form()`.

- [ ] **Step 2: Define the owned normalized assignment**

```c
typedef struct quantapdf_pdf_edit_form_assignment {
    quantapdf_form_value_presence presence;
    quantapdf_form_value_kind kind;
    char *utf8;
    size_t utf8_size;
    ...
    size_t value_count;
} quantapdf_pdf_edit_form_assignment;
```

`quantapdf_pdf_edit_form_assignment_drop()` frees `utf8` and `option_indices` and zeroes the struct.
...
const size_t value_min =
    offsetof(quantapdf_form_value_input, utf8_size) + sizeof(value->utf8_size);
const size_t update_min =
    offsetof(quantapdf_form_value_update, value_count) + sizeof(update->value_count);
```
...
if (pdf_is_dict(edit->ctx, acroform) &&
    quantapdf_pdf_dict_find(edit->ctx, acroform, PDF_NAME(XFA), &obj))
    return QUANTAPDF_ERROR_UNSUPPORTED;

if (pdf_is_dict(edit->ctx, acroform) &&
    quantapdf_pdf_dict_find(
...
quantapdf_pdf_form_build(..., want_provenance=1)
...
```c
if (assignment->presence == QUANTAPDF_FORM_VALUE_MISSING) {
    equal = field->value_presence == QUANTAPDF_FORM_VALUE_MISSING;
} else {
    const quantapdf_pdf_form_value_internal *current =
        &model->values[field->first_value];
    const char *current_text = model->strings + current->utf8.offset;
    equal = field->value_presence == QUANTAPDF_FORM_VALUE_PRESENT &&
        field->value_count == 1 &&
        current->kind == QUANTAPDF_FORM_VALUE_UTF8 &&
...
pdf_begin_operation(edit->ctx, edit->document,
    "QuantaPDF set form value");
operation_open = 1;
quantapdf_pdf_edit_form_write_semantic(...);
...
```bash
cmake --build build --parallel 2
ctest --test-dir build -R 'quantapdf\.(pdf_annotation_mutation|pdf_form|pdf_form_mutation)$' --output-on-failure
```
...
if (assignment->presence == QUANTAPDF_FORM_VALUE_PRESENT &&
...
for (model_widget_index = 0;
     ...
     ++model_widget_index) {
    const quantapdf_pdf_form_widget_internal *mw =
...
historical quantapdf_output captured before mutation stays byte-identical
```
...
After one successful setter, call `quantapdf_pdf_edit_snapshot()` twice. Reopen both outputs and assert identical normalized field values. No extra page load/update call is allowed between setter and snapshots.

- [ ] **Step 6: Run render + form regression tests**

```bash
cmake --build build --parallel 2
ctest --test-dir build -R 'quantapdf\.(render|pdf_form|pdf_form_mutation)$' --output-on-failure
```
...
Mirror internal names with `QUANTAPDF_PDF_EDIT_TEST_FAULT_...`.
...
ctest --test-dir build-asan -R '^quantapdf\.pdf_form_mutation$' --output-on-failure
```
...
```bash
git add include/quantapdf/quantapdf.h CMakeLists.txt src tests
...
Windows log must show `quantapdf.dll`, `quantapdf_test_pdf_form_mutation.exe`, and `quantapdf.pdf_form_mutation` test 21/21.
...
```text
docs/superpowers/specs/2026-08-29-quantapdf-acroform-value-mutation-design.md
docs/superpowers/plans/2026-08-29-quantapdf-acroform-value-mutation.md
include/quantapdf/quantapdf.h
CMakeLists.txt
