...
**Spec:** `docs/superpowers/specs/2026-08-28-quantapdf-output-save-file-design.md`
...
- `output` is non-consuming and immutable on success and failure. The caller continues to own it until `quantapdf_drop_output(...)`.
- `output == NULL`, `filename == NULL`, or `filename[0] == '\0'` returns `QUANTAPDF_ERROR_ARGUMENT`.
- Windows public paths are strict UTF-8: invalid UTF-8 returns `QUANTAPDF_ERROR_ARGUMENT`; temporary UTF-16 buffer allocation failure returns `QUANTAPDF_ERROR_NOMEM`.
...
- Open/write/flush/close failures return `QUANTAPDF_ERROR_IO`.
...
- On I/O failure the target may be absent, empty, truncated, or partially written; the input `quantapdf_output` remains valid and reusable.
- Do not change `struct quantapdf_output`, `src/pdf_export.c`, `src/pdf_output.c`, `src/pdf_merge.c`, or any PDF preservation/composition semantics.
...
tests/CMakeLists.txt
    register quantapdf.output_file
...
- Produces: one deterministic CTest `quantapdf.output_file` that specifies the public file-save contract before production code exists.
...
int main(void)
{
    ...
    CHECK(quantapdf_output_save_file(NULL, ASCII_OUTPUT_PDF) ==
          QUANTAPDF_ERROR_ARGUMENT);

    CHECK(quantapdf_open(COMPOSITION_PDF, NULL, &source) == QUANTAPDF_OK);
    CHECK(quantapdf_export_pages(source, indices, 2, &output) == QUANTAPDF_OK);
    CHECK(output != NULL);
    quantapdf_close(source);
    source = NULL;

    CHECK(quantapdf_output_data(output, &before_data, &before_size) ==
          QUANTAPDF_OK);
    CHECK(before_data != NULL);
    CHECK(before_size > 0);

    CHECK(quantapdf_output_save_file(output, NULL) ==
          QUANTAPDF_ERROR_ARGUMENT);
    CHECK(quantapdf_output_save_file(output, "") ==
          QUANTAPDF_ERROR_ARGUMENT);

    CHECK(write_stale_file(ASCII_OUTPUT_PDF, before_data, before_size));
    CHECK(quantapdf_output_save_file(output, ASCII_OUTPUT_PDF) ==
          QUANTAPDF_OK);
    ...
    CHECK(quantapdf_output_save_file(output, UTF8_OUTPUT_PDF) ==
          QUANTAPDF_OK);
    expect_saved_pdf(
        UTF8_OUTPUT_PDF,
        "PAGE-C", 300.0f, 150.0f,
    ...
    remove_missing_parent_if_empty();
    CHECK(quantapdf_output_save_file(output, MISSING_PARENT_PDF) ==
          QUANTAPDF_ERROR_IO);

#ifdef _WIN32
    {
        const char invalid_utf8[] = { (char)0xC3, (char)0x28, '\0' };
        CHECK(quantapdf_output_save_file(output, invalid_utf8) ==
              QUANTAPDF_ERROR_ARGUMENT);
    }
#endif

    CHECK(quantapdf_output_data(output, &after_data, &after_size) ==
          QUANTAPDF_OK);
...
The test deliberately does not use narrow Windows `fopen()` on `UTF8_OUTPUT_PDF`; it verifies that path only through the public UTF-8 `quantapdf_open(...)` API.
...
Add immediately after `quantapdf.pdf_merge`:
...
set(OUTPUT_FILE_MISSING_PARENT_DIR
  "${CMAKE_CURRENT_BINARY_DIR}/quantapdf-save-missing-parent")
set(OUTPUT_FILE_MISSING_PARENT
  "${OUTPUT_FILE_MISSING_PARENT_DIR}/output.pdf")

add_executable(quantapdf_test_output_file test_output_file.c)
target_link_libraries(quantapdf_test_output_file PRIVATE QuantaPDF::QuantaPDF)
target_compile_definitions(quantapdf_test_output_file PRIVATE
...
if(MSVC)
  target_compile_options(quantapdf_test_output_file PRIVATE /utf-8)
endif()
add_test(NAME quantapdf.output_file COMMAND quantapdf_test_output_file)
set_tests_properties(quantapdf.output_file PROPERTIES TIMEOUT 30)
```

Also append `quantapdf_test_output_file` to the existing `if(WIN32 AND BUILD_SHARED_LIBS)` target list so `quantapdf.dll` is copied beside the new test executable.
...
The body must state that the exact current head is intentional RED and that `quantapdf_output_save_file(...)` is not declared or implemented yet. Record the numeric PR number returned by GitHub and reuse that same PR throughout the remainder of the plan.
...
quantapdf_test_output_file         fails only because
                                    quantapdf_output_save_file is absent
```

Expected diagnostic class:

```text
implicit declaration of function 'quantapdf_output_save_file'
undefined reference to 'quantapdf_output_save_file'
```
...
**Interfaces:**
- Consumes: private `struct quantapdf_output { unsigned char *data; size_t size; }` from `src/internal.h`.
- Produces: public non-consuming `quantapdf_output_save_file(...)` with strict UTF-8 Windows path handling and exact stdio completion semantics.

- [ ] **Step 1: Add the public declaration**

In `include/quantapdf/quantapdf.h`, place the declaration after `quantapdf_output_data(...)` and before unrelated page APIs:
...
cmake --build build --target quantapdf_test_output_file --parallel 2
ctest --test-dir build -R '^quantapdf\.output_file$' --output-on-failure
```

Expected: `quantapdf.output_file` passes the ASCII exact-byte/truncate, UTF-8 filename, missing-parent I/O, and output-lifetime cases.
...
```text
docs/superpowers/specs/2026-08-28-quantapdf-output-save-file-design.md
docs/superpowers/plans/2026-08-28-quantapdf-output-save-file.md
include/quantapdf/quantapdf.h
src/output_file.c
CMakeLists.txt
tests/test_output_file.c
...
Windows:
  ...
  quantapdf_output_save_file exported      ✅
