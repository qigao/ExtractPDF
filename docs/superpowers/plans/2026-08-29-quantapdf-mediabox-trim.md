# QuantaPDF Immutable MediaBox Physical Trim V1 Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-08-29-quantapdf-mediabox-trim-design.md`

**Normative correction/evidence:** `docs/superpowers/specs/2026-08-29-quantapdf-mediabox-trim-frame-correction.md`

## Global Constraints

- Baseline master is `3fc48b5fb0f7a07926f7942fc4a4a3fb5e93a753`, tree `594499cfea3071f210b5b8781d73e942ba94a94d`.
- Public request coordinates are current QuantaPDF/Fitz page space; callers never supply raw PDF coordinates.
- MuPDF page matrix project invariant: PDF user space -> QuantaPDF/Fitz public page space; inverse maps public -> PDF.
...
- Encrypted and already-signed PDF inputs fail closed with `QUANTAPDF_ERROR_UNSUPPORTED`.
...
- Current suite is 22 CTests; this slice adds exactly one new CTest `quantapdf.pdf_trim`, yielding 23 total.
...
- `include/quantapdf/quantapdf.h` — approved `quantapdf_page_trim` and `quantapdf_trim_pages()` ABI only.
...
```c
quantapdf_page_trim trim = {0};
quantapdf_output *output = NULL;
trim.struct_size = sizeof(trim);
trim.page_index = 0;
trim.bounds = (quantapdf_rect){40,30,360,270};
...
```cmake
add_executable(quantapdf_test_pdf_trim test_pdf_trim.c)
target_link_libraries(quantapdf_test_pdf_trim PRIVATE QuantaPDF::QuantaPDF)
target_compile_definitions(quantapdf_test_pdf_trim PRIVATE
  TRIM_INTERACTIVE_PDF="${CMAKE_CURRENT_SOURCE_DIR}/fixtures/trim-interactive.pdf")
add_test(NAME quantapdf.pdf_trim COMMAND quantapdf_test_pdf_trim)
set_tests_properties(quantapdf.pdf_trim PROPERTIES TIMEOUT 60)
```
...
- [ ] Require Linux compile RED attributable only to missing `quantapdf_page_trim` / `quantapdf_trim_pages`; old 22 targets must still build. Stop otherwise.
...
{
    if (out_output == NULL) return QUANTAPDF_ERROR_ARGUMENT;
    *out_output = NULL;
...
- [ ] Require workflow: 23 executables compile; old 22 CTests pass; only `quantapdf.pdf_trim` runtime REDs at changed trim.
...
- [ ] Characterize `quantapdf.pdf_crop` before refactor; PASS required.
...
quantapdf_status quantapdf_pdf_trim_build_plan(
    ...
    const quantapdf_page_trim *trims, size_t trim_count,
    quantapdf_pdf_trim_plan *plans, int *out_any_changed);
```
...
```c
view = quantapdf_pdf_page_box_resolve(...);
...
- [ ] Add existing `full-ci` label to draft PR only; workflow unchanged. Require same frozen SHA, Linux static/sanitizer 23/23, macOS 23/23, Windows DLL 23/23. Windows log explicitly shows `quantapdf.dll`, `quantapdf_test_pdf_trim.exe`, `quantapdf.pdf_trim` as test 23/23, 100% of 23 tests.
...
- [ ] Require integrated-master exact merge SHA proof: Linux static 23/23, ASan/UBSan 23/23, macOS 23/23, Windows DLL 23/23 including `quantapdf_test_pdf_trim.exe`.
