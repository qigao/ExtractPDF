# QuantaPDF Structured Text Implementation Plan
...
**Architecture:** `quantapdf_extract_structured_text()` asks MuPDF for one `fz_stext_page`, then projects only text blocks into QuantaPDF-owned flat arrays and UTF-8 storage. Public callers traverse opaque snapshot data through count/info/text accessors; no public pointer references MuPDF memory after extraction returns.

**Tech Stack:** C11, MuPDF 1.28.2 Fitz structured text API, CMake/CTest, Linux ASan/UBSan, GitHub Actions Linux-first CI.

**Spec:** `docs/superpowers/specs/2026-08-27-quantapdf-structured-text-design.md`

## Global Constraints

- No MuPDF types or numeric MuPDF flags in `include/quantapdf/quantapdf.h`.
- Structured-text coordinates use the existing Fitz page-space contract: CropBox top-left origin, points, Y down.
- Snapshot lifetime is independent of `quantapdf_page` and `quantapdf_document` after successful extraction.
...
**Interfaces:**
- Consumes: existing `quantapdf_document`, `quantapdf_page`, `quantapdf_rect`, and page lifecycle.
- Produces the required public names for Task 2: `quantapdf_text_page`, `quantapdf_text_block_info`, `quantapdf_text_line_info`, `quantapdf_text_span_info`, `quantapdf_extract_structured_text`, block/line/span count/info accessors, `quantapdf_text_span_text`, and `quantapdf_drop_text_page`.
...
Add `quantapdf.structured_text` with a 30-second timeout and the fixture path compile definition. Include the target in Windows DLL staging even though Windows will remain skipped during Linux-first development.
...
Count accessors return `size_t`. Info accessors validate indices and `struct_size`, preserve the input `struct_size`, and fill the V1 fields. `quantapdf_text_span_text` returns a borrowed NUL-terminated snapshot-owned pointer with size excluding NUL.

- [ ] **Step 6: Implement cleanup and exception unwinding**

`quantapdf_drop_text_page(NULL)` is safe. Any allocation failure or caught MuPDF exception drops the upstream structured text and all partial QuantaPDF allocations. After successful return, the snapshot contains no pointer into MuPDF memory.

- [ ] **Step 7: Verify GREEN**

Run Linux build, full CTest, sanitizer build, and sanitizer CTest. Expected: `quantapdf.structured_text` and all existing tests pass with no sanitizer findings.
...
Test NULL snapshot/output arguments, out-of-range block/line/span indices, undersized `struct_size`, output resets (`count=0`, text pointer `NULL`, text size `0`), and `quantapdf_drop_text_page(NULL)`.
