# ExtractPDF Page Image Occurrence Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-08-27-extractpdf-page-images-design.md`
...
- Reuse existing public `extractpdf_quad` and Fitz page-space coordinates.
- Do not decode image pixels in this plan.
- Retain exact `fz_image` references for the future decode slice.
- Source page may be dropped; parent document must outlive `extractpdf_image_page`.
...
**Interfaces:**
- Consumes: existing `extractpdf_page`, `extractpdf_quad`, page/document lifetime.
- Produces required names for Task 2: `extractpdf_image_page`, `extractpdf_image_info`, `extractpdf_extract_images`, `extractpdf_image_count`, `extractpdf_image_get_info`, `extractpdf_drop_image_page`.
...
Add `extractpdf_test_images` to `tests/CMakeLists.txt`, define `PAGE_IMAGES_PDF` and `ONE_PAGE_PDF`, add `extractpdf.images` CTest, and include the test target in Windows post-build DLL copy staging.

- [ ] **Step 4: Verify RED**

Run Linux CI or compile the exact test against the pre-feature header. Expected failure: unknown `extractpdf_image_page` / `extractpdf_image_info` and missing `extractpdf_extract_images` / count/info/drop declarations.
...
Add an internal occurrence record with retained `fz_image *`, public quad and copied metadata. `extractpdf_image_page` stores a non-owning `extractpdf_document *document`, dynamic occurrence array, count and capacity.
...
Set only `super.fill_image`. The callback grows the array with overflow-safe `realloc`, calls `fz_keep_image`, copies metadata, and transforms the four unit-square corners through `ctm` into `extractpdf_quad`.

- [ ] **Step 4: Implement extraction lifecycle**

`extractpdf_extract_images` must set output NULL first, allocate a zero-count snapshot, construct the capture device, run `fz_run_page_contents(ctx, page->page, dev, fz_identity, NULL)` inside `fz_try/fz_always/fz_catch`, close/drop the device, unwind partial retained images on MuPDF/OOM failure, and return a valid snapshot on an image-free page.

- [ ] **Step 5: Implement traversal/error semantics**

`extractpdf_image_count` resets count before validation. `extractpdf_image_get_info` validates `struct_size`, zeroes known V1 fields on valid-sized argument/index failure while preserving `struct_size`, and copies one occurrence on success. `extractpdf_drop_image_page(NULL)` is safe and drops every retained image before freeing storage.
...
Run Linux build + all CTests, then ASan/UBSan build + all CTests. Expected: all existing tests plus `extractpdf.images` pass with no sanitizer findings.

- [ ] **Step 8: Commit GREEN**

```bash
git add include/extractpdf/extractpdf.h src/internal.h src/images.c CMakeLists.txt
...
Run `extractpdf.images` through Linux CI/CTest and capture the first failing assertion.
