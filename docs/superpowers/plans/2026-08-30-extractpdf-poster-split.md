# ExtractPDF Poster Split V1 Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-08-30-extractpdf-poster-split-design.md`

**Normative correction:** `docs/superpowers/specs/2026-08-30-extractpdf-poster-split-preflight-correction.md`
...
- Public Poster Split coordinates use the existing ExtractPDF/Fitz public page space; raw PDF coordinates are never a second row/column convention.
...
- Selected-page effective `/BleedBox`, `/TrimBox`, or `/ArtBox` makes a real split `EXTRACTPDF_ERROR_UNSUPPORTED`.
- A real split of tagged PDF (`Catalog /StructTreeRoot` or selected `/StructParents`) is `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
- Ordinary annotation/Widget crossing a grid boundary is `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
- Final suite target is exactly 24 CTests, adding `extractpdf.pdf_poster_split` while preserving all existing 23 tests.
...
- `include/extractpdf/extractpdf.h` — public request struct and immutable Poster Split function only; no MuPDF types.
...
```cmake
add_executable(extractpdf_test_pdf_poster_split test_pdf_poster_split.c)
target_link_libraries(extractpdf_test_pdf_poster_split PRIVATE ExtractPDF::ExtractPDF)
...
add_test(NAME extractpdf.pdf_poster_split COMMAND extractpdf_test_pdf_poster_split)
set_tests_properties(extractpdf.pdf_poster_split PROPERTIES TIMEOUT 60)

if(WIN32 AND BUILD_SHARED_LIBS)
  add_custom_command(TARGET extractpdf_test_pdf_poster_split POST_BUILD
    COMMAND ${CMAKE_COMMAND} -E copy_if_different
      $<TARGET_FILE:extractpdf>
      $<TARGET_FILE_DIR:extractpdf_test_pdf_poster_split>
    VERBATIM)
endif()
```
...
new target fails to compile only because:
  extractpdf_page_poster_split is unknown
  extractpdf_poster_split_pages is undeclared
```
...
CTest #1-#23 pass
CTest #24 extractpdf.pdf_poster_split fails at the valid 2×2 call because shell returns UNSUPPORTED
```

- [ ] **Step 6: Commit the ABI shell**

```bash
git add include/extractpdf/extractpdf.h src/pdf_poster.c CMakeLists.txt
...
**Interfaces:**
- Consumes: `extractpdf_pdf_page_box_resolve()` from `pdf_page_box_common.h`.
- Produces:
...
4. resolve strict page box view through `extractpdf_pdf_page_box_resolve`;
...
typedef enum extractpdf_pdf_poster_annot_kind {
    EXTRACTPDF_PDF_POSTER_ANNOT_LINK,
    EXTRACTPDF_PDF_POSTER_ANNOT_WIDGET,
    EXTRACTPDF_PDF_POSTER_ANNOT_ORDINARY
...
Use `extractpdf_pdf_annotation_classify()` for ordinary subtype mapping where possible, but do not inherit immutable enumeration's tolerant-ignore semantics for objects that Poster Split must move.
...
```c
extractpdf_status extractpdf_pdf_poster_rect_tiles(
...
On any mismatch return `EXTRACTPDF_ERROR_FORMAT` before page-graph mutation.
...
extractpdf_status extractpdf_pdf_poster_apply_annotations(
    ...
    extractpdf_pdf_poster_private_split *private_splits);
```
...
typedef enum extractpdf_pdf_poster_dest_owner_kind {
    EXTRACTPDF_PDF_POSTER_DEST_LINK,
    EXTRACTPDF_PDF_POSTER_DEST_OUTLINE,
...
Windows log must explicitly show `extractpdf.dll`, `extractpdf_test_pdf_crop.exe`, `extractpdf_test_pdf_trim.exe`, and `extractpdf_test_pdf_poster_split.exe` built, with Poster Split as test 24/24 and 100% of 24 tests passing.
...
macOS ARM64 24/24
Windows DLL 24/24
Windows built extractpdf.dll and extractpdf_test_pdf_poster_split.exe
```
