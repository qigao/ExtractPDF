...
**Spec:** `docs/superpowers/specs/2026-08-28-quantapdf-pdf-annotation-mutation-design.md`

**Base:** integrated master `58525ce1b4b691ef53dfafc7c4e4d82753c966ba`; branch `feat/pdf-annotation-mutation`; issue #37; roadmap #2.

## Global Constraints

- Keep `quantapdf_document`, `quantapdf_page`, and all existing immutable snapshots read-only; no mutation API writes through those handles.
- `quantapdf_pdf_edit_begin()` becomes independent of the source lifetime after success.
...
- `snapshot()` is non-consuming, deterministic for unchanged editor state, leaves refs valid, and returns an independent existing `quantapdf_output`.
...
- Modify `include/quantapdf/quantapdf.h`
  - add opaque `quantapdf_pdf_edit`;
  - add value `quantapdf_annotation_ref`;
  - add update-field/create/update types;
  - append `QUANTAPDF_ERROR_STATE = 8`;
  - add begin/discovery/getter/CRUD/snapshot/drop APIs.
- Modify `src/status.c`
  - add stable status text for `QUANTAPDF_ERROR_STATE`.

### Shared private annotation semantics

- Create `src/pdf_annotation_common.h`
  - private `quantapdf_pdf_annotation_view`;
...
- Create `src/pdf_edit_internal.h`
  - private editor struct, ref registry, token helpers, page/object resolution helpers, test-fault constants under `QUANTAPDF_TESTING`.
...
- Create `tests/pdf_edit_fault_hook.c`
  - test-only hook compiled into `quantapdf` only when tests are built; state remains per editor, never global.
- Create:
...
The unknown `quantapdf_pdf_edit` type is intentionally part of RED.
...
CHECK(quantapdf_pdf_edit_annotation_count(NULL, 0, &count) == QUANTAPDF_ERROR_ARGUMENT);
CHECK(count == 0);
CHECK(quantapdf_pdf_edit_annotation_count(NULL, 0, NULL) == QUANTAPDF_ERROR_ARGUMENT);

CHECK(quantapdf_pdf_edit_annotation_ref_at(NULL, 0, 0, &ref) == QUANTAPDF_ERROR_ARGUMENT);
CHECK(ref_is_zero(&ref));

info.struct_size = offsetof(quantapdf_annotation_info, flags);
CHECK(quantapdf_pdf_edit_annotation_get_info(NULL, &ref, &info) == QUANTAPDF_ERROR_ARGUMENT);

text = (char *)(uintptr_t)1;
size = 99;
CHECK(quantapdf_pdf_edit_annotation_contents(NULL, &ref, &text, &size) == QUANTAPDF_ERROR_ARGUMENT);
CHECK(text == NULL && size == 0);

output = (quantapdf_output *)&sentinel;
CHECK(quantapdf_pdf_edit_snapshot(NULL, &output) == QUANTAPDF_ERROR_ARGUMENT);
CHECK(output == NULL);
CHECK(quantapdf_pdf_edit_snapshot(NULL, NULL) == QUANTAPDF_ERROR_ARGUMENT);

quantapdf_drop_pdf_edit(NULL);
CHECK(strcmp(quantapdf_status_string(QUANTAPDF_ERROR_STATE), "invalid state") == 0);
```
...
CHECK(quantapdf_open(ENCRYPTED_PDF, "user-pass", &source) == QUANTAPDF_OK);
edit = (quantapdf_pdf_edit *)(uintptr_t)1;
CHECK(quantapdf_pdf_edit_begin(source, &edit) == QUANTAPDF_ERROR_UNSUPPORTED);
CHECK(edit == NULL);
quantapdf_close(source);
...
CHECK(quantapdf_open(UNSIGNED_SIGNATURE_PDF, NULL, &source) == QUANTAPDF_OK);
CHECK(quantapdf_pdf_edit_begin(source, &edit) == QUANTAPDF_OK);
quantapdf_drop_pdf_edit(edit);
quantapdf_close(source);
...
```c
CHECK(quantapdf_pdf_edit_annotation_count(a, 0, &count) == QUANTAPDF_OK);
CHECK(count == 5);
CHECK(quantapdf_pdf_edit_annotation_ref_at(a, 0, 0, &text_ref) == QUANTAPDF_OK);
CHECK(quantapdf_pdf_edit_annotation_ref_at(a, 0, 1, &square_ref) == QUANTAPDF_OK);
CHECK(quantapdf_pdf_edit_annotation_ref_at(a, 0, 1, &square_again) == QUANTAPDF_OK);
...
```c
count = 99;
CHECK(quantapdf_pdf_edit_annotation_count(edit, 0, &count) == QUANTAPDF_ERROR_FORMAT);
CHECK(count == 0);
ref.opaque[0] = UINT64_MAX;
ref.opaque[1] = UINT64_MAX;
CHECK(quantapdf_pdf_edit_annotation_ref_at(edit, 0, 0, &ref) == QUANTAPDF_ERROR_FORMAT);
CHECK(ref_is_zero(&ref));
```
...
size_t after_size = 0;
quantapdf_annotation_update change = {0};

before.struct_size = sizeof(before);
CHECK(quantapdf_pdf_edit_annotation_get_info(edit, &square_ref, &before) == QUANTAPDF_OK);
...
change.fields = QUANTAPDF_ANNOTATION_UPDATE_BOUNDS |
                QUANTAPDF_ANNOTATION_UPDATE_CONTENTS;
change.bounds = (quantapdf_rect){20,20,90,90};
...
CHECK(quantapdf_pdf_edit_annotation_update(edit, &square_ref, &change) != QUANTAPDF_OK);

after.struct_size = sizeof(after);
CHECK(quantapdf_pdf_edit_annotation_get_info(edit, &square_ref, &after) == QUANTAPDF_OK);
CHECK(quantapdf_pdf_edit_annotation_contents(
...
```c
CHECK(quantapdf_pdf_edit_annotation_count(edit, 0, &before_count) == QUANTAPDF_OK);
quantapdf_test_pdf_edit_set_fault(
    edit, QUANTAPDF_TEST_PDF_EDIT_FAULT_AFTER_CREATE_MUTATION);
zero_ref(&new_ref);
CHECK(quantapdf_pdf_edit_annotation_create(edit, 0, &options, &new_ref) != QUANTAPDF_OK);
CHECK(ref_is_zero(&new_ref));
CHECK(quantapdf_pdf_edit_annotation_count(edit, 0, &after_count) == QUANTAPDF_OK);
...
CHECK(quantapdf_pdf_edit_annotation_get_info(edit, &text_ref, &info) == QUANTAPDF_OK);
CHECK(info.flags == UINT32_C(2147483649));

update.struct_size = sizeof(update);
update.fields = QUANTAPDF_ANNOTATION_UPDATE_FLAGS;
update.flags = UINT32_MAX;
CHECK(quantapdf_pdf_edit_annotation_update(edit, &text_ref, &update) == QUANTAPDF_OK);
CHECK(quantapdf_pdf_edit_annotation_get_info(edit, &text_ref, &info) == QUANTAPDF_OK);
...
CHECK(quantapdf_output_data(repeat, &repeat_data, &repeat_size) == QUANTAPDF_OK);
...
add_executable(quantapdf_test_pdf_annotation_mutation
  test_pdf_annotation_mutation.c)
target_link_libraries(quantapdf_test_pdf_annotation_mutation PRIVATE QuantaPDF::QuantaPDF)
target_compile_definitions(quantapdf_test_pdf_annotation_mutation PRIVATE
...
add_test(NAME quantapdf.pdf_annotation_mutation
  COMMAND quantapdf_test_pdf_annotation_mutation)
set_tests_properties(quantapdf.pdf_annotation_mutation PROPERTIES TIMEOUT 60)
```
...
only quantapdf_test_pdf_annotation_mutation fails to compile
...
```bash
ctest --test-dir build -R '^quantapdf\.pdf_annotations$' --output-on-failure
```
...
cmake --build build --parallel 2 --target \
  ...
  quantapdf_test_pdf_outline \
  quantapdf_test_pdf_annotations
ctest --test-dir build -E '^quantapdf\.pdf_annotation_mutation$' --output-on-failure
```
...
**Interfaces:**
- Consumes: `quantapdf_serialize_pdf()`, private output bytes, MuPDF PDF stream/JS/journal/signature APIs.
...
QUANTAPDF_API quantapdf_status quantapdf_pdf_edit_begin(
    quantapdf_document *, quantapdf_pdf_edit **);
QUANTAPDF_API quantapdf_status quantapdf_pdf_edit_annotation_count(
    quantapdf_pdf_edit *, int, size_t *);
QUANTAPDF_API quantapdf_status quantapdf_pdf_edit_annotation_ref_at(
    quantapdf_pdf_edit *, int, size_t, quantapdf_annotation_ref *);
QUANTAPDF_API quantapdf_status quantapdf_pdf_edit_annotation_get_info(
    quantapdf_pdf_edit *, const quantapdf_annotation_ref *, quantapdf_annotation_info *);
QUANTAPDF_API quantapdf_status quantapdf_pdf_edit_annotation_contents(
    quantapdf_pdf_edit *, const quantapdf_annotation_ref *, char **, size_t *);
QUANTAPDF_API quantapdf_status quantapdf_pdf_edit_annotation_create(
    quantapdf_pdf_edit *, int, const quantapdf_annotation_create_options *, quantapdf_annotation_ref *);
QUANTAPDF_API quantapdf_status quantapdf_pdf_edit_annotation_update(
    quantapdf_pdf_edit *, const quantapdf_annotation_ref *, const quantapdf_annotation_update *);
QUANTAPDF_API quantapdf_status quantapdf_pdf_edit_annotation_delete(
    quantapdf_pdf_edit *, const quantapdf_annotation_ref *);
QUANTAPDF_API quantapdf_status quantapdf_pdf_edit_snapshot(
    quantapdf_pdf_edit *, quantapdf_output **);
QUANTAPDF_API void quantapdf_drop_pdf_edit(quantapdf_pdf_edit *);
```
...
quantapdf_serialize_pdf(source ctx/pdf) -> deterministic seed_output
allocate edit
...
Any exception tears down partial state and maps through `quantapdf_status_from_mupdf()`.

- [ ] **Step 6: Implement non-consuming snapshot with `pdf_write_snapshot()`**

Do not call full-finalizing `quantapdf_serialize_pdf()` on live editor. MuPDF 1.28.2 `pdf_write_snapshot()` is specifically the non-finalizing writer for the in-memory incremental xref.

In `src/pdf_edit.c`, create a private buffer/output, call `pdf_write_snapshot()`, close output, retrieve buffer storage, allocate `quantapdf_output`, deep-copy bytes, then publish only after complete success. The helper must drop buffer/output on every exception path and map MuPDF errors.

Required skeleton:

```c
static quantapdf_status snapshot_pdf(
    quantapdf_pdf_edit *edit,
    quantapdf_output **out_output)
{
    ...
    *out_output = NULL;
    ...
    if (caught != FZ_ERROR_NONE) {
        ...
        return quantapdf_status_from_mupdf(caught);
    }
    if (data == NULL || size == 0) {
...
`quantapdf_pdf_edit_snapshot()` resets output, validates editor, calls helper, then under test build consumes `SNAPSHOT_BEFORE_PUBLISH` by dropping result and returning `QUANTAPDF_ERROR_MUPDF`. Same-state byte identity RED is mandatory acceptance proof; never weaken it.

- [ ] **Step 7: Implement drop and annotation API validation shells**

`quantapdf_drop_pdf_edit()` drops kept registry objects, registry, private PDF document, context, then seed output; NULL no-op. Document must be dropped before context/seed bytes.
...
```cmake
target_sources(quantapdf PRIVATE
...
Production source compares only private `QUANTAPDF_PDF_EDIT_TEST_FAULT_*` constants.
...
cmake -S . -B build \
  ...
  -DBUILD_SHARED_LIBS=OFF
cmake --build build --parallel 2
./build/tests/quantapdf_test_pdf_annotation_mutation arguments
./build/tests/quantapdf_test_pdf_annotation_mutation begin
```
...
```bash
git add include/quantapdf/quantapdf.h src/status.c \
...
```bash
./build/tests/quantapdf_test_pdf_annotation_mutation discovery
```
...
```c
static quantapdf_status scan_page(
    quantapdf_pdf_edit *edit,
    int page_index,
    size_t wanted_index,
    int want_object,
...
```c
static quantapdf_status resolve_live_annot(
    quantapdf_pdf_edit *edit,
...
```bash
./build/tests/quantapdf_test_pdf_annotation_mutation create
```
...
```bash
./build/tests/quantapdf_test_pdf_annotation_mutation update-delete
./build/tests/quantapdf_test_pdf_annotation_mutation contents-flags
```
...
```c
pdf_begin_operation(ctx, edit->document, "QuantaPDF delete annotation");
...
```bash
./build/tests/quantapdf_test_pdf_annotation_mutation snapshot
```
...
```bash
./build/tests/quantapdf_test_pdf_annotation_mutation javascript
```
...
```bash
cmake --build build --parallel 2
./build/tests/quantapdf_test_pdf_annotation_mutation
```
...
```text
docs/superpowers/specs/2026-08-28-quantapdf-pdf-annotation-mutation-design.md
docs/superpowers/plans/2026-08-29-quantapdf-pdf-annotation-mutation.md
tests/CMakeLists.txt
tests/pdf_edit_test_api.h
