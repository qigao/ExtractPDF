# ExtractPDF Immutable CropBox Transform V1 Implementation Plan
...
**Architecture:** Keep the source `extractpdf_document` immutable. Strictly preflight all requests against the source PDF, serialize the complete source, reopen it in a fresh private MuPDF context with JavaScript disabled, write only page-local `/CropBox` entries for changed requests, deterministically serialize the private document, and publish an independent `extractpdf_output`. Do not use page grafting, content translation, session-local editor refs, page-wide form update, or interactive runtime execution.
...
**Spec:** `docs/superpowers/specs/2026-08-29-extractpdf-cropbox-transform-design.md`
...
- Current suite baseline is 21 CTests; this feature adds exactly one new CTest named `extractpdf.pdf_crop`, yielding 22 CTests.
...
- Encrypted input and already-signed input are `EXTRACTPDF_ERROR_UNSUPPORTED`.
...
- `include/extractpdf/extractpdf.h` — public request type and `extractpdf_crop_pages()` declaration only.
- `src/pdf_crop_internal.h` — crop-private resolved-page/request-plan structs and strict-preflight helper declarations; no public types beyond consuming `extractpdf_page_crop`.
...
- `tests/test_pdf_crop_raw.c` — test-only MuPDF parser for structural assertions about local `/CropBox` presence/absence and untouched dictionaries. It is compiled into the same `extractpdf_test_pdf_crop` executable.
...
**Interfaces:**
- Consumes: existing public `extractpdf_document`, `extractpdf_page`, `extractpdf_output`, `extractpdf_rect`, open/page/output APIs.
- Produces: a test-only dependency on the approved but still-absent public symbols `extractpdf_page_crop` and `extractpdf_crop_pages()`; draft PR tracking for #49.
...
```cmake
add_executable(extractpdf_test_pdf_crop test_pdf_crop.c)
target_link_libraries(extractpdf_test_pdf_crop PRIVATE ExtractPDF::ExtractPDF)
target_compile_definitions(extractpdf_test_pdf_crop PRIVATE
  CROP_INTERACTIVE_PDF="${CMAKE_CURRENT_SOURCE_DIR}/fixtures/crop-interactive.pdf")
add_test(NAME extractpdf.pdf_crop COMMAND extractpdf_test_pdf_crop)
set_tests_properties(extractpdf.pdf_crop PROPERTIES TIMEOUT 60)
```
...
Expected: the `extractpdf` library and every existing test executable #1-#21 build; only `extractpdf_test_pdf_crop` fails because `extractpdf_page_crop` and/or `extractpdf_crop_pages` do not exist. There must be no fixture parse/runtime failure yet because the new target must not link.
...
**Interfaces:**
- Consumes: public `extractpdf_rect`, `extractpdf_document`, `extractpdf_output`.
- Produces:
...
Place `extractpdf_page_crop` near the existing geometry/output public structures and add `extractpdf_crop_pages()` alongside the immutable composition/output functions. Do not add transform enums/options or MuPDF-facing types.
...
Expected: all 22 executables build; existing #1-#21 pass; only `extractpdf.pdf_crop` fails with `valid crop failed` because the shell returns `EXTRACTPDF_ERROR_UNSUPPORTED`.

- [ ] **Step 5: Commit the ABI shell**

```bash
git add include/extractpdf/extractpdf.h src/pdf_crop.c CMakeLists.txt
...
typedef struct extractpdf_pdf_crop_page_view {
    ...
    extractpdf_rect visible_public; /* effective visible region in public space */
    int rotate_degrees;
    float user_unit;
    ...
    extractpdf_status expected)
{
    ...
    extractpdf_status status =
        extractpdf_crop_pages(document, crops, count, &output);
...
Extend `target_compile_definitions(extractpdf_test_pdf_crop ...)` with:
...
If the intersection has non-positive width/height, return `EXTRACTPDF_ERROR_FORMAT`.
...
For each `extractpdf_page_crop`:

1. validate `struct_size >= offsetof(extractpdf_page_crop, bounds) + sizeof(extractpdf_rect)`;
2. validate finite ordered public bounds;
3. validate page index and uniqueness;
4. resolve `extractpdf_pdf_crop_page_view`;
...
Allocate `crop_count` plan entries with overflow checks. Run security preflight and `extractpdf_pdf_crop_build_plan()` before any serialization.
...
If `any_changed != 0`, continue returning `EXTRACTPDF_ERROR_UNSUPPORTED` for now so the changed-crop case remains the next attributable RED.
...
Expected: old #1-#21 pass; validation/security/malformed/no-op assertions inside `extractpdf.pdf_crop` pass; the target still fails only at the first changed crop with `valid crop failed`.
...
**Interfaces:**
- Consumes: `extractpdf_pdf_crop_build_plan()` and `extractpdf_pdf_crop_resolve_page()` from Task 3; deterministic `extractpdf_serialize_pdf()`.
...
```c
extractpdf_page_crop crops[2] = {
    { sizeof(extractpdf_page_crop), 0, { 50.0f, 40.0f, 350.0f, 260.0f } },
    { sizeof(extractpdf_page_crop), 1, { 20.0f, 30.0f, 380.0f, 270.0f } }
};
```
...
2. obtain output bytes with `extractpdf_output_data()`;
...
```cmake
add_executable(extractpdf_test_pdf_crop
  test_pdf_crop.c
  test_pdf_crop_raw.c)
target_link_libraries(extractpdf_test_pdf_crop PRIVATE
...
```text
1. extractpdf_serialize_pdf(source ctx, source_pdf, &seed_output)
2. fz_new_context(...)
...
10. extractpdf_serialize_pdf(private_ctx, private_document, out_output)
...
```c
static void extractpdf_pdf_crop_put_box(
    fz_context *ctx,
    pdf_obj *page_obj,
    fz_rect box)
{
...
At this checkpoint deliberately reject changed pages whose resolved transform includes nonzero Rotate or non-default UserUnit with `EXTRACTPDF_ERROR_UNSUPPORTED`; Task 5 removes this temporary implementation limitation after its RED fixtures are added. Do not reject no-op requests solely due to Rotate/UserUnit.
...
Expected: rotated or UserUnit changed crop returns the temporary `EXTRACTPDF_ERROR_UNSUPPORTED`, making the new case the attributable RED; all existing unrotated crop assertions still pass before it.
...
For one changed two-page batch, call `extractpdf_crop_pages()` twice on the same unchanged source and compare:
...
```text
docs/superpowers/specs/2026-08-29-extractpdf-cropbox-transform-design.md
docs/superpowers/plans/2026-08-29-extractpdf-cropbox-transform.md
include/extractpdf/extractpdf.h
CMakeLists.txt
src/pdf_crop.c
...
For Windows logs, explicitly verify `extractpdf.dll`, `extractpdf_test_pdf_crop.exe`, and `extractpdf.pdf_crop` as test 22/22 with 100% of 22 CTests passing.
...
Inspect Windows logs for `extractpdf.dll`, `extractpdf_test_pdf_crop.exe`, test 22/22, and `100% tests passed out of 22`.
