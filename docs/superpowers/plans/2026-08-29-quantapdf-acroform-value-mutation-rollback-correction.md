# QuantaPDF AcroForm Value Mutation #266 Correction Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Correct workflow #266 by making `quantapdf_form_field_ref` identity independent of journal-sensitive MuPDF object incarnations and proving failed form mutations are byte-identical, ref-stable, and editor-reusable.
...
- The original plan `docs/superpowers/plans/2026-08-29-quantapdf-acroform-value-mutation.md` remains historical record for Tasks 1-8. This plan supersedes its Task 9+ identity/rollback implementation where the corrected spec differs.
...
- A syntactically valid session ref whose locator cannot resolve after a successful strict parse returns `QUANTAPDF_ERROR_STATE`.
...
```c
quantapdf_form_field_ref rediscovered = rollback_field_ref(edit, field_name);
...
- Produces: `quantapdf_pdf_form_live_field.locator` for every public terminal field.

- [ ] **Step 1: Add the private locator type**

In `src/pdf_form_common.h`, add before `quantapdf_pdf_form_live_field`:
...
Add before `quantapdf_pdf_form_materialize_provenance()`:
...
Inside the terminal-group loop in `quantapdf_pdf_form_materialize_provenance()`, add `quantapdf_status status;` to the loop-local declarations, then immediately after assigning `live` add:
...
In `quantapdf_pdf_form_drop_provenance()`, for each field add:
...
```bash
cmake --build build --target quantapdf_test_pdf_form quantapdf_test_pdf_form_mutation --parallel 2
ctest --test-dir build -R '^quantapdf\.pdf_form$' --output-on-failure
ctest --test-dir build -R '^quantapdf\.pdf_form_mutation$' --output-on-failure
```

Required: `quantapdf.pdf_form` passes; mutation remains RED at the pointer-backed registry invariant.
...
**Interfaces:**
- Consumes: Task 2 `quantapdf_pdf_form_live_field.locator` and fresh current provenance objects.
...
edit->form_entries[slot].tag =
    quantapdf_pdf_edit_form_tag_for_slot(edit, slot);
...
In `quantapdf_pdf_edit_form_resolve_ref()` replace `group_head` validity with:
...
In `quantapdf_pdf_edit_form_find_current_field()` keep the existing `quantapdf_pdf_form_build(..., 1, ...)` call, **remove the current pre-scan call** to `quantapdf_pdf_form_capture_provenance_widgets()`, and replace the `group_head` identity scan with:
...
```bash
ctest --test-dir build -R '^quantapdf\.(pdf_form|pdf_form_mutation)$' --output-on-failure
```
...
```bash
git log -1 --format=%H -- docs/superpowers/plans/2026-08-29-quantapdf-acroform-value-mutation-rollback-correction.md
...
Windows must show `quantapdf.dll`, `quantapdf_test_pdf_form_mutation.exe`, and `quantapdf.pdf_form_mutation` built/ran.
...
```text
docs/superpowers/plans/2026-08-29-quantapdf-acroform-value-mutation-rollback-correction.md
