    ...
    const float expected[4])
{
    ...
    fz_try(ctx)
    {
        ...
        present = dict_has_local_key(ctx, page, PDF_NAME(CropBox), &box);
        if (present != expect_present) {
            ok = 0;
        } else if (!expect_present) {
            ok = 1;
        } else if (expected == NULL || !pdf_is_array(ctx, box) ||
    ...
    int page_index)
{
...

    {
        ...
        for (index = 0; index < left_count; ++index) {
            ...
            static pdf_obj *keys[] = {
                PDF_NAME(Subtype), PDF_NAME(Rect), PDF_NAME(F), PDF_NAME(Contents),
                PDF_NAME(A), PDF_NAME(Dest), PDF_NAME(AS), NULL
            };
            int key_index;
            ...
            if (pdf_name_eq(
                    ctx,
                    pdf_dict_get(ctx, left_annot, PDF_NAME(Subtype)),
                    PDF_NAME(Widget))) {
                pdf_obj *left_parent = pdf_dict_get(
                    ctx, left_annot, PDF_NAME(Parent));
                pdf_obj *right_parent = pdf_dict_get(
                    ctx, right_annot, PDF_NAME(Parent));
                static pdf_obj *field_keys[] = {
                    PDF_NAME(FT), PDF_NAME(T), PDF_NAME(V), PDF_NAME(Ff), NULL
                };
                int field_index;

                if (!pdf_is_dict(ctx, left_parent) ||
                    !pdf_is_dict(ctx, right_parent))
                    return 0;
                for (field_index = 0;
                     field_keys[field_index] != NULL;
                     ++field_index) {
                    if (!obj_equal_deep_or_both_missing(
                            ctx,
                            pdf_dict_get(
                                ctx, left_parent, field_keys[field_index]),
                            pdf_dict_get(
                                ctx, right_parent, field_keys[field_index])))
                        return 0;
                }
            }
        }
    ...
    pdf_document *after)
{
    ...
    if (left_acroform != NULL) {
        ...
        if (pdf_array_len(ctx, left_fields) != pdf_array_len(ctx, right_fields))
            return 0;
        if (pdf_array_len(ctx, left_fields) > 0) {
            pdf_obj *left_field = pdf_array_get(ctx, left_fields, 0);
            pdf_obj *right_field = pdf_array_get(ctx, right_fields, 0);
            static pdf_obj *keys[] = {
                PDF_NAME(FT), PDF_NAME(T), PDF_NAME(V), PDF_NAME(Ff), NULL
            };
            int index;
            for (index = 0; keys[index] != NULL; ++index) {
                if (!obj_equal_deep_or_both_missing(
                        ctx,
                        pdf_dict_get(ctx, left_field, keys[index]),
                        pdf_dict_get(ctx, right_field, keys[index])))
                    return 0;
            }
        }
    }
