...
static void check_surviving_ancestor_output(const extractpdf_output *output)
{
    ...
    fz_try(ctx)
    {
        ...
        RAW_CHECK(pdf_is_array(ctx, root_kids));
        RAW_CHECK(pdf_array_len(ctx, root_kids) == 1);
        RAW_CHECK(pdf_is_array(ctx, audit_root_kids));
        RAW_CHECK(pdf_array_len(ctx, audit_root_kids) == 1);
        RAW_CHECK(same_identity(ctx, root_kids, audit_root_kids));

        mid = pdf_array_get(ctx, root_kids, 0);
        RAW_CHECK(pdf_is_dict(ctx, mid));
        mid_kids = pdf_dict_get(ctx, mid, PDF_NAME(Kids));
        RAW_CHECK(pdf_is_array(ctx, mid_kids));
        RAW_CHECK(pdf_array_len(ctx, mid_kids) == 1);
        RAW_CHECK(pdf_is_array(ctx, audit_mid_kids));
        RAW_CHECK(pdf_array_len(ctx, audit_mid_kids) == 2);
        RAW_CHECK(!same_identity(ctx, mid_kids, audit_mid_kids));
        RAW_CHECK(same_identity(ctx, pdf_array_get(ctx, mid_kids, 0), keep_ref));
        RAW_CHECK(same_identity(
            ctx, pdf_array_get(ctx, audit_mid_kids, 1), keep_ref));
...
int extractpdf_test_pdf_flatten_form_ancestor_survives(void)
{
    ...
    CHECK(extractpdf_open(
        FLATTEN_FORM_ANCESTOR_SURVIVES_PDF, NULL, &document) == EXTRACTPDF_OK);
    CHECK(document != NULL);
    ...
    CHECK(output != NULL);
    check_surviving_ancestor_output(output);
